...
// --- Game Objects & State ---
let ship;
let bullets = []; let homingMissiles = []; // REMOVED: laserBeams = [];
...
// --- Menu Variables ---
let menuItems = ['Start Game', 'Settings', 'Cosmetics'];
...
// REMOVED: mobileLaserButton = { x: 0, y: 0, size: 0, padding: 5 };
...
function calculateMobileActionButtonsPosition() { let buttonSize = isMobile ? 50 : 60; let padding = 15; mobileMissileButton.size = buttonSize; mobileMissileButton.padding = padding; mobileMissileButton.x = width - buttonSize - padding; mobileMissileButton.y = height - buttonSize - padding; // REMOVED: Laser button positioning
}
...
function draw() {
    ...
    switch (gameState) { case GAME_STATE.START_MENU: displayStartMenu(); break; case GAME_STATE.SETTINGS_MENU: displaySettingsMenu(); break; case GAME_STATE.COSMETICS_MENU: displayCosmeticsMenu(); break; // Changed to display new cosmetics menu
...
function runGameLogic() {
    if (isPaused) { if (ship) ship.draw(); if (ship && ship.droneActive) ship.drone.draw(); for (let b of bullets) b.draw(); for (let m of homingMissiles) m.draw(); /* REMOVED: Laser loop */ for (let p of particles) p.draw(); for (let a of asteroids) a.draw(); for (let e of enemyShips) e.draw(); for (let eb of enemyBullets) eb.draw(); for (let pt of potions) pt.draw(); for (let pu of powerUps) pu.draw(); displayHUD(); return; }
    if (!ship) return;
    ...
    // REMOVED: Laser damage application block
...
function resetGame() {
    ...
    bullets = []; homingMissiles = []; /* REMOVED: laserBeams = []; */ particles = []; asteroids = []; potions = []; enemyShips = []; enemyBullets = []; powerUps = []; nebulas = []; shootingStars = [];
...
function startNextLevel() {
    ...
    asteroids = []; bullets = []; homingMissiles = []; /* REMOVED: laserBeams = []; */ enemyShips = []; enemyBullets = []; powerUps = []; potions = [];
...
function keyPressed() {
    ...
    else if (gameState === GAME_STATE.PLAYING && !isPaused && ship) { if (keyCode === 32) { if (!spacebarHeld) { spacebarHeld = true; } return false; } if (keyCode === 77) { ship.fireMissile(); return false; } /* REMOVED: Laser keybinding (keyCode 76) */ }
...
function touchStarted() {
    ...
    let setBtn = mobileSettingsButton; let misBtn = mobileMissileButton; // REMOVED: lasBtn
    ...
    if (gameState === GAME_STATE.PLAYING && !isPaused && ship) {
        ...
        // REMOVED: Laser button touch check
    }
...
class Ship {
    ...
    getUpgradeCost(upgradeType) { let level, maxLevel, multiplier = this.costMultiplier; switch (upgradeType) { case 'fireRate': level = this.fireRateLevel; maxLevel = this.maxUpgradeLevel; break; case 'spreadShot': level = this.spreadShotLevel; maxLevel = this.maxUpgradeLevel; break; case 'rearGun': level = this.rearGunLevel; maxLevel = this.maxRearGunLevel; multiplier = this.costMultiplier * 0.8; break; case 'homingMissiles': level = this.homingMissilesLevel; maxLevel = this.maxMissileLevel; multiplier = this.specialCostMultiplier; break; // REMOVED: Laser Beam case
        default: return Infinity; } if (level >= maxLevel) return "MAX"; return floor(this.baseUpgradeCost * pow(multiplier, level)); }
    attemptUpgrade(upgradeType) { let cost = this.getUpgradeCost(upgradeType); if (typeof cost !== 'number' || money < cost) return false; money -= cost; switch (upgradeType) { case 'fireRate': this.fireRateLevel++; break; case 'spreadShot': this.spreadShotLevel++; break; case 'rearGun': this.rearGunLevel++; break; case 'homingMissiles': this.homingMissilesLevel++; this.maxMissiles = this.missileCapacity[this.homingMissilesLevel]; break; // REMOVED: Laser Beam case
        default: money += cost; return false; } return true; }
    resetPositionForNewLevel() { this.pos.set(width / 2, height - 50); this.vel.set(0, 0); this.invulnerableTimer = 60; this.rapidFireTimer = 0; this.tempShieldActive = false; // REMOVED: Laser reset logic
        this.missileCooldown = 0; this.scoreMultiplierTimer = 0; this.scoreMultiplierValue = 1; this.droneActive = false; this.drone = null; this.invincibilityTimer = 0; }
    update() {
        if (this.invulnerableTimer > 0) { this.invulnerableTimer--; } if (this.rapidFireTimer > 0) { this.rapidFireTimer--; } if (this.shootCooldown > 0) { this.shootCooldown--; } if (this.missileCooldown > 0) { this.missileCooldown--; } // REMOVED: Laser cooldown decrement
        ...
        // REMOVED: Laser active/duration check
        ...
        // --- Handle Auto-Fire ---
        // MODIFIED: Added check for isMobileShooting flag, Removed laser check
