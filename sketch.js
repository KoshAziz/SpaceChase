...
// - Controls Menu (Displays Keyboard & Mobile Controls, Back) // <<< NEW FEATURE // MODIFIED: Text Alignment
...
// - Asteroid Splitting
// - Player Lives (Max 3, +1 for Mobile) // MODIFIED FOR MOBILE
...
// - Mobile Adjustments: Lower base asteroid spawn rate. // ENHANCED (UI Scaling/Layout) // FURTHER ADJUSTED FOR EASIER MOBILE PLAY
...
// - MODIFIED: Increased mobile touch thrust multiplier slightly for easier control. // << EASIER MOBILE ADJUSTMENT
// - MODIFIED: Further reduced mobile difficulty factor (spawn rates). // << EASIER MOBILE ADJUSTMENT
// - MODIFIED: Added +1 starting life for mobile players. // << EASIER MOBILE ADJUSTMENT
// - MODIFIED: Aligned text within Controls menu panel. // << TEXT ALIGNMENT ADJUSTMENT
// --------------------------
...
let spacebarHeld = false; // <<< Keep track of spacebar state
...
function setDifficultyForLevel(level) {
    ...
    let mobileFactor = isMobile ? 0.6 : 1.0; // Reduced from 0.7
...
function setupControlsMenuButton() {
    ...
    controlsBackButton.y = height * 0.85; // Position near the bottom (Adjusted slightly higher)
}
...
// <<< MODIFIED: Controls Menu Display Function with Text Alignment >>>
function displayControlsMenu() {
    let panelW = width * (isMobile ? 0.9 : 0.7);
    let panelH = height * 0.75; // Increased height slightly
    let panelX = width / 2 - panelW / 2;
    let panelY = height / 2 - panelH / 2;

    drawPanelBackground(panelW, panelH);

    fill(uiTextColor);
    textSize(isMobile ? 36 : 48);
    textAlign(CENTER, TOP);
    text("Controls", width / 2, panelY + panelH * 0.08); // Adjusted title Y

    let startY = panelY + panelH * 0.22; // Adjusted start Y
    let lineSpacing = isMobile ? 23 : 28; // Adjusted spacing
    let sectionSpacing = isMobile ? 35 : 45; // Adjusted spacing
    let controlTextSize = isMobile ? 14 : 16;
    let headerTextSize = isMobile ? 18 : 20;

    // Define X coordinates relative to the panel
    let nameX = panelX + panelW * 0.1;
    let descX = panelX + panelW * 0.4;

    // --- Keyboard Controls ---
    textSize(headerTextSize);
    textAlign(LEFT, TOP);
    fill(uiHighlightColor); // Header color
    text("Keyboard:", nameX, startY);

    textSize(controlTextSize);
    fill(uiTextColor); // Regular text color
    startY += sectionSpacing;

    text("- Move:", nameX, startY);
    text("Arrow Keys / WASD", descX, startY);
    startY += lineSpacing;

    text("- Shoot:", nameX, startY);
    text("Spacebar (Hold to Charge)", descX, startY);
    startY += lineSpacing;

    text("- Missile:", nameX, startY);
    text("M Key", descX, startY);
    startY += lineSpacing;

    text("- Laser:", nameX, startY);
    text("L Key", descX, startY);
    startY += lineSpacing;

    text("- Pause:", nameX, startY);
    text("ESC Key", descX, startY);
    startY += lineSpacing;

    text("- Menus:", nameX, startY);
    text("Arrow Keys + Enter", descX, startY);
    ...
    text("- Move:", nameX, startY);
    text("Drag Finger", descX, startY);
    startY += lineSpacing;

    text("- Shoot:", nameX, startY);
    text("Tap Screen (Not Buttons)", descX, startY);
    startY += lineSpacing;

    text("- Missile:", nameX, startY);
    text("💥 Button (Bottom Right)", descX, startY);
    startY += lineSpacing;
    ...
    text("- Pause:", nameX, startY);
    text("⚙️ Button (Bottom Left)", descX, startY);
    startY += lineSpacing;

    text("- Menus:", nameX, startY);
    text("Tap Buttons", descX, startY);

    // Back Button (Position updated based on new panel height)
    controlsBackButton.y = panelY + panelH - controlsBackButton.h - panelH * 0.05;
    controlsBackButton.x = width / 2 - controlsBackButton.w / 2; // Center horizontally
...
// --- Game State Control ---
// <<< MODIFIED: resetGame gives extra life on mobile >>>
function resetGame() {
    ship = new Ship();
    bullets = []; chargeShots = []; homingMissiles = []; laserBeams = []; particles = []; asteroids = []; potions = []; enemyShips = []; enemyBullets = []; powerUps = []; nebulas = []; shootingStars = [];
    points = 0; money = 0;
    lives = isMobile ? MAX_LIVES + 1 : MAX_LIVES; // Mobile gets an extra life
    currentLevel = 1;
    setDifficultyForLevel(currentLevel);
    lastPlanetAppearanceTime = -Infinity; planetVisible = false; frameCount = 0; infoMessage = ""; infoMessageTimeout = 0; screenShakeDuration = 0; screenShakeIntensity = 0; isPaused = false; levelTransitionFlash = 0; comboCounter = 0; comboTimer = 0; maxComboReached = 0; showComboText = false; comboTextTimeout = 0; spacebarHeld = false; cursor();
...
// Ship Class
// <<< MODIFIED: Increased touchThrustMultiplier >>>
class Ship {
    constructor() {
        this.pos = createVector(width / 2, height - 50);
        this.vel = createVector(0, 0);
        this.thrust = 0.38;
        this.touchThrustMultiplier = 1.35; // Increased from 1.15
        this.friction = 0.975;
        this.maxSpeed = 9.5;
        this.size = 30;
    ...
    resetPositionForNewLevel() { this.pos.set(width / 2, height - 50); this.vel.set(0, 0); this.invulnerableTimer = 60; this.rapidFireTimer = 0; this.tempShieldActive = false; this.isCharging = false; this.chargeLevel = 0; this.laserActive = false; this.laserDuration = 0; this.laserCooldown = 0; this.missileCooldown = 0; this.scoreMultiplierTimer = 0; this.scoreMultiplierValue = 1; this.droneActive = false; this.drone = null; this.invincibilityTimer = 0; }
