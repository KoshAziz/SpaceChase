...
// --- Game Objects & State ---
let ship;
let bullets = []; /* REMOVED chargeShots = [] */ let homingMissiles = []; let laserBeams = [];
...
function setupShopButtons() {
    ...
    // Removed Charge Shot Button - Shifting subsequent buttons up
...
// --- Main Game Logic ---
// MODIFIED: Removed chargeShots loop
function runGameLogic() {
    if (isPaused) {
        // Draw static elements when paused
        if (ship) ship.draw();
        if (ship && ship.droneActive) ship.drone.draw();
        for (let b of bullets) b.draw();
        // Removed chargeShots drawing
        ...
        displayHUD(); // Still show HUD when paused
        return;
    }

    if (!ship) return; // Should not happen in PLAYING state after resetGame

    // --- Update Game Objects ---
    ship.update();
    if (ship.droneActive) ship.drone.update();
    ship.draw(); // Draw ship first
    if (ship.droneActive) ship.drone.draw(); // Draw drone
    ...
    // Removed chargeShots update/draw loop

    // Homing Missiles
    for (let i = homingMissiles.length - 1; i >= 0; i--) {
        homingMissiles[i].update(); homingMissiles[i].draw();
        if (homingMissiles[i].isOffscreen() || homingMissiles[i].lifespan <= 0) { homingMissiles.splice(i, 1); }
    }

    // Laser Beams (Visual only, damage is handled differently)
    for (let i = laserBeams.length - 1; i >= 0; i--) {
        laserBeams[i].update(); laserBeams[i].draw();
        if (laserBeams[i].isDead()) { laserBeams.splice(i, 1); }
    }
    ...
    for (let i = enemyShips.length - 1; i >= 0; i--) {
        enemyShips[i].update(); enemyShips[i].draw();
        if (enemyShips[i].isOffscreen()) { enemyShips.splice(i, 1); }
    }
    ...
    for (let i = asteroids.length - 1; i >= 0; i--) {
        ...
        asteroids[i].update(); asteroids[i].draw();
    }

    // --- Special Weapon Logic ---
    // Laser Damage (Applied continuously while active)
    if (ship.laserActive) {
        let laserDamage = 0.05 + ship.laserBeamLevel * 0.02; // Damage per frame? Adjust as needed.
        ...
        for (let i = enemyShips.length - 1; i >= 0; i--) {
             ...
             // Check if enemy is roughly in front of the ship (within laser cone)
             if (abs(angleToEnemy + PI/2) < PI / 10 && d > 0) { // PI/10 radians ≈ 18 degrees cone width
                 // Check collision using distance to line segment (more accurate than point check)
                 let projectedPoint = p5.Vector.sub(enemy.pos, laserOrigin);
                 projectedPoint.rotate(-(-PI/2)); // Rotate so laser direction is y-axis
                 let distToLine = abs(projectedPoint.x); // Distance from enemy center to laser line

                 if (distToLine < (enemy.size / 2 + laserWidth / 2)) { // Collision detected
                     if (enemy.takeDamage(laserDamage)) { // Apply damage
                         ...
                         // Hit effect if not destroyed
                         if(frameCount % 5 === 0) { // Don't create particles every frame
                     ...
                     // Optional: Make lower level lasers not pierce (uncomment break)
                     // if (ship.laserBeamLevel < 3) {
                     //     break; // Stop checking enemies after hitting one
                     // }
                 }
             }
        }
    ...
    if (comboTimer > 0) {
        comboTimer--;
        if (comboTimer <= 0) {
            if (maxComboReached >= 3) { // Award bonus only if combo was significant
                 let bonusPoints = maxComboReached * 5; let bonusMoney = floor(maxComboReached / 3);
                 points += bonusPoints; money += bonusMoney;
                 infoMessage = `Combo Bonus: +${bonusPoints} PTS, +$${bonusMoney}! (Max: x${maxComboReached})`;
                 infoMessageTimeout = 120;
            }
            comboCounter = 0; maxComboReached = 0; showComboText = false; comboTextTimeout = 0; // Reset combo
        }
    }
    ...
    if (gameState === GAME_STATE.PLAYING) { // Only spawn during active play
         ...
         // Spawn Limits per frame / total on screen
         ...
         if (random(1) < currentTotalEnemySpawnRate && enemyShips.length < maxEnemiesAllowed) {
              ...
              if (typeRoll < swarmerWeight && currentLevel >= 3) { // Swarmers (spawn in groups)
                  let swarmCount = floor(random(5, 10));
                  let spawnX = random(width * 0.2, width * 0.8); let spawnY = -20;
                  for (let i = 0; i < swarmCount; i++) {
                       ...
                       } else break; // Stop spawning if max enemies reached
                  }
              } else if (typeRoll < swarmerWeight + turretWeight && currentLevel >= 5) { // Turrets
                  enemyShips.push(new TurretEnemy());
              } else if (typeRoll < swarmerWeight + turretWeight + kamikazeWeight && currentLevel >= 2) { // Kamikaze
        ...
        // Background Nebulas
        if (settingBackgroundEffectsEnabled && random(1) < nebulaSpawnRate && nebulas.length < maxNebulasAllowed) {
...
// MODIFIED: Removed chargeShot collision checks and bonus
function handlePowerUps() {
    if (gameState !== GAME_STATE.PLAYING || isPaused) {
        for (let i = powerUps.length - 1; i >= 0; i--) { powerUps[i].draw(); } // Draw static powerups if paused
        return;
    }
    if (!ship) return;
        ...
        if (powerUps[i].hitsShip(ship)) {
            ...
            switch (powerUpType) {
                ...
                case POWERUP_TYPES.EMP_BURST:
                    ...
                    createParticles(ship.pos.x, ship.pos.y, 60, color(210, 100, 100), 12, 3.5, 1.2); // Player burst effect
                    // Destroy asteroids & enemies
                    for (let k = asteroids.length - 1; k >= 0; k--) { createParticles(asteroids[k].pos.x, asteroids[k].pos.y, 15, asteroids[k].color, 3, 1.5); }
                    asteroids = [];
                    for (let k = enemyShips.length - 1; k >= 0; k--) { createParticles(enemyShips[k].pos.x, enemyShips[k].pos.y, 20, enemyShips[k].getExplosionColor()); }
                    enemyShips = [];
                ...
                case POWERUP_TYPES.DRONE:
                    if (!ship.droneActive) { ship.droneActive = true; ship.drone = new Drone(ship); infoMessage = "DRONE ACTIVATED!"; createParticles(ship.pos.x, ship.pos.y, 30, color(180, 50, 100)); }
                    else { infoMessage = "DRONE ALREADY ACTIVE!"; } // Or maybe refresh timer?
                    break;
                case POWERUP_TYPES.INVINCIBILITY: ship.invincibilityTimer = 480; infoMessage = "INVINCIBLE!"; createParticles(ship.pos.x, ship.pos.y, 40, color(0, 0, 100)); ship.tempShieldActive = false; ship.shieldCharges = 0; break; // Invincibility overrides other shields
            }
...
function handleCollisions() {
    ...
    // Helper to handle enemy destruction, points, money, combo
    const destroyEnemy = (enemy, index, projectileType) => {
        let basePoints = enemy.pointsValue;
        let baseMoney = enemy.moneyValue;
        // Removed ChargeShot bonus check
        if (projectileType === 'HomingMissile') { basePoints *= 1.1; } // Missile bonus
    ...
    for (let i = asteroids.length - 1; i >= 0; i--) {
        ...
        for (let j = bullets.length - 1; j >= 0; j--) {
            if (asteroids[i] && bullets[j] && asteroids[i].hits(bullets[j])) {
                createParticles(bullets[j].pos.x, bullets[j].pos.y, floor(random(3, 6)), color(60, 40, 100), 2, 0.8, 0.7); // Bullet impact particle
                bullets.splice(j, 1); // Remove bullet
                asteroidHit = true; break; // Asteroid was hit, move to processing it
            }
        }
        // Removed ChargeShot vs Asteroid check

        // Check Missiles vs Asteroid
        if (!asteroidHit) { // Only check if not already hit by a bullet this frame
             for (let j = homingMissiles.length - 1; j >= 0; j--) {
                  if (asteroids[i] && homingMissiles[j] && asteroids[i].hits(homingMissiles[j])) {
                      createParticles(homingMissiles[j].pos.x, homingMissiles[j].pos.y, 15, homingMissiles[j].color, 5, 1.8, 1.0); // Missile impact
                      homingMissiles.splice(j, 1); // Remove missile
        ...
        if (asteroidHit) {
            ...
            // Explosion effect and removal
            let asteroidPos = asteroids[i].pos.copy(); let asteroidColor = asteroids[i].color;
            ...
            if (asteroidSizeValue > minAsteroidSize * 2) {
                ...
                let vel1 = p5.Vector.random2D().mult(splitSpeedMultiplier); let vel2 = p5.Vector.random2D().mult(splitSpeedMultiplier);
            ...
            if (currentLevel < MAX_LEVEL && points >= LEVEL_THRESHOLDS[currentLevel]) {
                points += 100 * currentLevel; money += 25 * currentLevel; // Level completion bonus
                ...
                return; // Exit collision loop early as we change state
            }
            // --- Win Condition Check ---
    ...
    for (let i = enemyShips.length - 1; i >= 0; i--) {
        ...
        for (let j = bullets.length - 1; j >= 0; j--) {
            if (bullets[j] && enemy.hits(bullets[j])) {
                createParticles(bullets[j].pos.x, bullets[j].pos.y, 5, color(0,0,100), 2); // Bullet hit particle
                bullets.splice(j, 1); // Remove bullet
                if (enemy.takeDamage(1)) { // Apply damage
                    destroyEnemy(enemy, i, 'Bullet'); // Handle destruction
                    enemyDestroyed = true;
                } else {
                    createParticles(enemy.pos.x, enemy.pos.y, 3, enemy.getHitColor(), 2); // Hit particle
                }
        ...
        // Removed ChargeShot vs Enemy check

        // Check Missiles vs Enemy
        for (let j = homingMissiles.length - 1; j >= 0; j--) {
             if (homingMissiles[j] && enemy.hits(homingMissiles[j])) {
                 createParticles(homingMissiles[j].pos.x, homingMissiles[j].pos.y, 20, homingMissiles[j].color, 6, 2.0, 1.2); // Missile explosion
                 let missileDamage = homingMissiles[j].damage || 3; // Get damage from missile (fallback 3)
                 homingMissiles.splice(j, 1); // Remove missile
                 if (enemy.takeDamage(missileDamage)) { // Apply damage
                     destroyEnemy(enemy, i, 'HomingMissile');
                     enemyDestroyed = true;
                 } else {
                     createParticles(enemy.pos.x, enemy.pos.y, 8, enemy.getHitColor(), 4); // Hit particle
                 }
    ...
    if (ship.invulnerableTimer <= 0 && ship.invincibilityTimer <= 0) { // Check player collisions only if not invulnerable

        // Internal helper for taking damage
        const takeDamage = (sourceObject, sourceArray, index) => {
            if (ship.invincibilityTimer > 0 || ship.invulnerableTimer > 0) return false; // Double check invulnerability
            ...
            if (comboCounter > 0) {
                 if (maxComboReached >= 3) {
                      let bonusPoints = maxComboReached * 5; let bonusMoney = floor(maxComboReached / 3);
                      points += bonusPoints; money += bonusMoney; // Award partial bonus on break
            ...
            if (ship.tempShieldActive) {
                ...
                infoMessage = "TEMPORARY SHIELD LOST!"; infoMessageTimeout = 90;
                // Optional: destroy the source if it's a bullet
                if (sourceObject instanceof EnemyBullet && sourceArray && index !== undefined) {
                ...
                lives--;
                createParticles(ship.pos.x, ship.pos.y, 40, color(0, 90, 100), 5, 2.2); // Player hit effect
                if (settingScreenShakeEnabled) { screenShakeIntensity = 7; screenShakeDuration = 120; } // Screen shake on life loss

                if (lives <= 0) {
                    gameState = GAME_STATE.GAME_OVER; infoMessage = ""; infoMessageTimeout = 0; cursor(ARROW);
                    gameOver = true;
                } else {
                    ship.setInvulnerable(); // Grant brief invulnerability after hit
                }

                // Destroy the source object (asteroid/enemy/bullet) that hit the player
                if (sourceObject && sourceArray && index !== undefined && sourceArray.includes(sourceObject)) {
                    ...
                    // Kamikaze explodes differently and is removed, others are just removed
        ...
        for (let i = asteroids.length - 1; i >= 0; i--) {
            if (asteroids[i] && asteroids[i].hitsShip(ship)) {
                if (takeDamage(asteroids[i], asteroids, i)) return; // If game over, exit
                break; // Only take damage from one source per frame
            }
        }

        // Check Enemies vs Player
        if (gameState === GAME_STATE.PLAYING) { // Ensure we haven't changed state
             for (let i = enemyShips.length - 1; i >= 0; i--) {
                 let enemy = enemyShips[i];
                 if (enemy && enemy.hitsShip(ship)) {
                     if (takeDamage(enemy, enemyShips, i)) return;
                     break;
                 }
             }
        }
        ...
        if (gameState === GAME_STATE.PLAYING) {
             for (let i = enemyBullets.length - 1; i >= 0; i--) {
                 if (enemyBullets[i] && enemyBullets[i].hitsShip(ship)) {
                     if (takeDamage(enemyBullets[i], enemyBullets, i)) return;
                     break;
                 }
             }
        }
    } // End player collision check (if not invulnerable)
}
function handlePotions() {
    if (gameState !== GAME_STATE.PLAYING || isPaused) {
        for (let i = potions.length - 1; i >= 0; i--) { potions[i].draw(); } // Draw static potions if paused
        return;
    }
    if (!ship) return;
    ...
    for (let i = potions.length - 1; i >= 0; i--) {
        ...
        if (potions[i].hitsShip(ship)) {
            createParticles(potions[i].pos.x, potions[i].pos.y, 20, color(0, 80, 100), 4, 1.5); // Potion pickup effect
            if (lives < MAX_LIVES) {
                lives++; infoMessage = "+1 LIFE!"; infoMessageTimeout = 90;
            } else { // Award points if lives are full
                let pointsToAdd = 25;
                if(ship.scoreMultiplierTimer > 0) { pointsToAdd *= ship.scoreMultiplierValue; }
                points += pointsToAdd;
                infoMessage = `+${pointsToAdd} POINTS (MAX LIVES)!`; infoMessageTimeout = 90;
            }
...
function drawBackgroundAndStars() {
    ...
    if (settingBackgroundEffectsEnabled) {
         ...
         for (let i = nebulas.length - 1; i >= 0; i--) {
             if (gameState === GAME_STATE.PLAYING && !isPaused) nebulas[i].update(); // Only update in play
             nebulas[i].draw();
             if (nebulas[i].isOffscreen()) { nebulas.splice(i, 1); }
         }
         // Static Background Features
         drawBlackHole(); drawGalaxy();
         // Planets (appear and move)
         if (planetVisible) { drawPlanet(); }
         // Shooting Stars
         for (let i = shootingStars.length - 1; i >= 0; i--) {
             // Update unless paused during gameplay
             if (!(gameState === GAME_STATE.PLAYING && isPaused)) shootingStars[i].update();
             shootingStars[i].draw();
             if (shootingStars[i].isDone()) { shootingStars.splice(i, 1); }
         }
         // Regular Stars (Parallax + Twinkle)
         for (let star of stars) {
             if (!(gameState === GAME_STATE.PLAYING && isPaused)) star.update();
             star.draw();
         }
    } else { // Draw simple stars if effects are off
        fill(0, 0, 80, 70); noStroke();
        for (let star of stars) {
             ellipse(star.x, star.y, 1.5, 1.5);
             // Simple parallax even when effects off
             if (!(gameState === GAME_STATE.PLAYING && isPaused)) {
                  star.y += star.speed * 0.5; // Slower scroll
                  if (star.y > height + 2) { star.y = -2; star.x = random(width); }
             }
        }
    }
}
...
// --- HUD & Info Messages ---
// MODIFIED: displayHUD removes CHRG text
// MODIFIED: displayInfoMessage removes charge bar height dependency
function displayHUD() {
    ...
    if (isMobile && gameState === GAME_STATE.PLAYING && !isPaused && ship) {
        ...
        if (ship.homingMissilesLevel > 0) {
            let btn = mobileMissileButton; let btnColor = uiButtonColor; let textColor = uiTextColor; let borderColor = uiButtonBorderColor; let icon = '🚀'; let subText = `${ship.currentMissiles}`; let isDisabled = ship.currentMissiles <= 0 || ship.missileCooldown > 0;
            if (isDisabled) { btnColor = uiButtonDisabledColor; textColor = color(0, 0, 60); borderColor = color(0, 0, 40); if (ship.missileCooldown > 0) { subText = `${ceil(ship.missileCooldown / 60)}`; } }
        ...
        if (ship.laserBeamLevel > 0) {
            let btn = mobileLaserButton; let btnColor = uiButtonColor; let textColor = uiTextColor; let borderColor = uiButtonBorderColor; let icon = '⚡'; let subText = ``; let isDisabled = ship.laserActive || ship.laserCooldown > 0;
            if (isDisabled) { btnColor = uiButtonDisabledColor; textColor = color(0, 0, 60); borderColor = color(0, 0, 40); if (ship.laserActive) { subText = `${ceil(ship.laserDuration / 60)}`; btnColor = color(0, 80, 70); } else if (ship.laserCooldown > 0) { subText = `${ceil(ship.laserCooldown / 60)}`; } }
            fill(btnColor); stroke(borderColor); strokeWeight(1.5); rect(btn.x, btn.y, btn.size, btn.size, 8);
            textSize(btn.size * 0.5); fill(textColor); noStroke(); text(icon, btn.x + btn.size / 2, btn.y + btn.size * 0.45);
            if(subText !== '') { textSize(btn.size * 0.25); fill(textColor); text(subText, btn.x + btn.size / 2, btn.y + btn.size * 0.8); }
        }
    }
}
function displayInfoMessage() {
    ...
    // Removed barH dependency (related to charge bar)
    let boxY = height - boxH - (isMobile? 15 : 30); // Position above bottom edge
...
// --- Game State Control ---
// MODIFIED: resetGame removes chargeShots
function resetGame() {
    ship = new Ship();
    bullets = []; /* REMOVED chargeShots */ homingMissiles = []; laserBeams = [];
...
function startNextLevel() {
    ...
    if (ship.homingMissilesLevel > 0) { ship.currentMissiles = ship.maxMissiles; } // Refill missiles
    // Clear transient objects for new level
    asteroids = []; bullets = []; /* REMOVED chargeShots */ homingMissiles = []; laserBeams = [];
    enemyShips = []; enemyBullets = []; powerUps = []; potions = [];
    frameCount = 0; infoMessage = `Starting Level ${currentLevel}`; infoMessageTimeout = 90;
    levelTransitionFlash = 15; // Visual flash effect
    spawnInitialAsteroids();
...
// --- Input Handling ---
// MODIFIED: Input functions no longer reference charge shot
...
function keyPressed() {
    if (keyCode === ESCAPE) { if (gameState === GAME_STATE.PLAYING) { isPaused = !isPaused; if (isPaused) {cursor(ARROW); previousGameState = gameState;} else {cursor();} } else if (gameState === GAME_STATE.SETTINGS_MENU) { selectSettingsItemAction(settingsItems.findIndex(item => item.id === 'back')); } }
        ...
        if (keyCode === 32) { // Spacebar - Set flag, auto-fire handled in update
            if (!spacebarHeld) { spacebarHeld = true; }
             // Optional: uncomment below for single shot on first press even with auto-fire
             // ship.shoot();
            return false;
        }
        if (keyCode === 77) { ship.fireMissile(); return false; } // M
        if (keyCode === 76) { ship.fireLaser(); return false; } // L
    }
...
function touchStarted() {
    ...
    // Gameplay Touch - Shoot (if not hitting other buttons)
    else if (gameState === GAME_STATE.PLAYING && !isPaused && ship) {
        let hitUI = false;
        ...
        if (!hitUI) { ship.shoot(); } // Single shot on tap
        return false;
    }
    return false;
}
function touchEnded() { /* No action needed on touch end anymore */ return false; }
...
// Ship Class
// MODIFIED: Removed all charge shot properties and methods
class Ship {
    constructor() {
        ...
        // Removed Charge Shot properties
        this.rearGunLevel = 0;
        ...
        this.specialCostMultiplier = 2.5; // For Missiles/Laser
    ...
    // MODIFIED: getUpgradeCost removed charge shot
    ...
    // MODIFIED: attemptUpgrade removed charge shot
    attemptUpgrade(upgradeType) {
        ...
        switch (upgradeType) {
            ...
            case 'homingMissiles': this.homingMissilesLevel++; this.maxMissiles = this.missileCapacity[this.homingMissilesLevel]; break;
            case 'laserBeam': this.laserBeamLevel++; break;
            default: money += cost; return false;
        }
        return true;
    }

    // MODIFIED: resetPositionForNewLevel removed charge shot state
    resetPositionForNewLevel() {
        this.pos.set(width / 2, height - 50); this.vel.set(0, 0);
        this.invulnerableTimer = 60; this.rapidFireTimer = 0; this.tempShieldActive = false;
        // Removed isCharging / chargeLevel reset
        this.laserActive = false; this.laserDuration = 0; this.laserCooldown = 0;
        this.missileCooldown = 0; this.scoreMultiplierTimer = 0; this.scoreMultiplierValue = 1;
        this.droneActive = false; this.drone = null; this.invincibilityTimer = 0;
    }

    // MODIFIED: update method removed charging logic, simplified auto-fire condition
    update() {
        // Timers
        ...
        if (isTouching) { // Touch Drag Movement
             ...
             let currentThrust = this.thrust; if (!isMobile) { currentThrust *= 1.5; }
             ...
             if (movingDown) { acceleration.y += currentThrust; }
        ...
        if (applyThrustParticles && frameCount % 3 === 0) { let thrustColor = lerpColor(this.baseEngineColor1, color(60, 100, 100), this.fireRateLevel / this.maxUpgradeLevel); createParticles(this.pos.x, this.pos.y + this.size * 0.6, 1, thrustColor, 3, 1.5, 0.5); }
        ...
        // Simplified condition: Only check if space is held and laser is not active
        if (!isMobile && keyIsDown(32) && gameState === GAME_STATE.PLAYING && !isPaused && !this.laserActive) {
            this.shoot(); // Attempt to shoot (respects shootCooldown)
        }
    }

    shoot() {
        // Condition changed: Removed !this.isCharging check
    ...
    // Removed startCharging() method
    // Removed releaseChargeShot() method

    fireMissile() {
        // Condition changed: Removed !this.laserActive check (allow missiles during laser?) - RE-ADDED !laserActive for consistency
    ...
    fireLaser() {
        // Condition changed: Removed !this.isCharging check
         if (this.laserBeamLevel > 0 && !this.laserActive && this.laserCooldown <= 0) {
             this.laserActive = true; this.laserDuration = this.laserDurationTime[this.laserBeamLevel];
             laserBeams.push(new LaserBeam(this, this.laserColor, 5 + this.laserBeamLevel * 1.5));
             createParticles(this.pos.x, this.pos.y - this.size*0.7, 30, this.laserColor, 5, 1.0, 0.5);
         }
    }
    ...
    // MODIFIED: draw method removed charging indicator logic
    draw() {
        ...
        if (drawShip) {
            ...
            // Draw Ship Body (unchanged)
            let s = this.size; let bodyW = s * 0.5; let wingW = s * (this.shapeState === 0 ? 1.1 : 1.3); let wingH = s * 0.8; let noseL = s * 0.8; let pointsHue = (200 + points * 0.2) % 360; let bodyColor = color(pointsHue, 85, 98); let wingColor = color(hue(bodyColor), saturation(bodyColor) * 0.8, brightness(bodyColor) * 0.7);
            ...
            // Removed Charging Indicator drawing logic
            pop();
        }
    }
}
