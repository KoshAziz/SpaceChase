...
// --- MODIFICATION: Updated mobile settings button position calculation ---
function calculateMobileSettingsButtonPosition() {
    mobileSettingsButton.size = isMobile ? 35 : 45;
    mobileSettingsButton.padding = 10; // Increased padding slightly
    mobileSettingsButton.x = mobileSettingsButton.padding; // Position from left edge
    mobileSettingsButton.y = height - mobileSettingsButton.size - mobileSettingsButton.padding; // Position from bottom edge
}
// ----------------------------------------------------------------------
...
function touchStarted() { if (touches.length === 0) return false; let touchX = touches[0].x; let touchY = touches[0].y; let btn = mobileSettingsButton; if (gameState === GAME_STATE.PLAYING && !isPaused && isMobile && touchX > btn.x && touchX < btn.x + btn.size + btn.padding*2 && touchY > btn.y && touchY < btn.y + btn.size + btn.padding*2) { /* MODIFIED: Slightly larger hit area */ isPaused = true; previousGameState = gameState; gameState = GAME_STATE.SETTINGS_MENU; selectedSettingsItem = 0; cursor(ARROW); return false; } else if (gameState === GAME_STATE.START_MENU) { for (let i = 0; i < startMenuButtons.length; i++) { let button = startMenuButtons[i]; if (touchX > button.x && touchX < button.x + button.w && touchY > button.y && touchY < button.y + button.h) { selectedMenuItem = i; selectMenuItem(i); return false; } } return false; } else if (gameState === GAME_STATE.SETTINGS_MENU) { for (let i = 0; i < settingsMenuButtons.length; i++) { let button = settingsMenuButtons[i]; if (touchX > button.x && touchX < button.x + button.w && touchY > button.y && touchY < button.y + button.h) { selectedSettingsItem = i; selectSettingsItemAction(i); return false; } } return false; } else if (gameState === GAME_STATE.GAME_OVER || gameState === GAME_STATE.WIN_SCREEN) { previousGameState = gameState; gameState = GAME_STATE.START_MENU; selectedMenuItem = 0; return false; } else if (gameState === GAME_STATE.UPGRADE_SHOP) { for (let button of shopButtons) { if (touchX > button.x && touchX < button.x + button.w && touchY > button.y && touchY < button.y + button.h) { handleShopButtonPress(button.id); return false; } } return false; } else if (gameState === GAME_STATE.PLAYING && !isPaused) { if (!isMobile || !(touchX > btn.x - btn.padding && touchX < btn.x + btn.size + btn.padding*2 && touchY > btn.y - btn.padding && touchY < btn.y + btn.size + btn.padding*2)) { ship.shoot(); } return false; } return false; }
...
class Ship {
    ...
    draw() {
        if (this.invulnerableTimer <= 0 || (this.invulnerableTimer > 0 && frameCount % 10 < 5)) {
            ...
            let enginePulseFactor = 1.0 + this.vel.mag() * 0.04; let pulseSpeed = (this.rapidFireTimer > 0) ? 0.5 : 0.25; let enginePulse = map(sin(frameCount * pulseSpeed), -1, 1, 0.8, 1.3) * enginePulseFactor; let engineSize = this.size * 0.55 * enginePulse; let engineBrightness = map(sin(frameCount * 0.35), -1, 1, 85, 100); noStroke(); let engineColor1 = lerpColor(this.baseEngineColor1, color(60, 90, 100), this.fireRateLevel / this.maxLevel); let engineColor2 = lerpColor(this.baseEngineColor2, color(45, 90, 100), this.fireRateLevel / this.maxLevel); let engineY = this.size * 0.6;
...
class Asteroid {
    constructor(x, y, size, vel) {
        ...
        if (isInitialPlacement) {
            ...
            let edge = floor(random(3)); // Only spawn top, left, right
            if (edge === 0) { // Top
        ...
        for (let i = 0; i < numVertices; i++) {
            ...
            let r = this.size / 2 + random(-this.size * 0.45, this.size * 0.35); // More variation
        ...
        for (let i = 0; i < numCraters; i++) {
            ...
            let radius = random(this.size * 0.1, this.size * 0.4); // Distribute craters wider
    ...
    update() {
        ...
        this.rotationSpeed = constrain(this.rotationSpeed, -0.06, 0.06);
    ...
    draw() {
        ...
        // Apply a simple gradient for shading/highlighting
        let gradSteps = 10;
        let bodyRadius = this.size / 2;
        for (let i = 0; i < gradSteps; i++) {
            let inter = i / gradSteps;
            let y = lerp(-bodyRadius, bodyRadius, inter);
            let h = lerp(0, bodyRadius * 2, inter);
        ...
        for (let crater of this.craters) {
            fill(craterBaseColor);
            ellipse(crater.pos.x, crater.pos.y, crater.size, crater.size * random(0.7, 1.3)); // Base crater
            // Add subtle highlight inside
             fill(craterHighlightColor);
             ellipse(crater.pos.x + crater.size * 0.1, crater.pos.y + crater.size * 0.1, crater.size * 0.6, crater.size * 0.6 * random(0.7, 1.3));
        }
    ...
    hitsShip(ship) {
        ...
        // Slightly reduced asteroid collision radius for fairness
        return d < this.size / 2 * 0.9 + targetRadius;
    }
}
// ==================
// Particle Class
