...
let powerUps = [];
let nebulas = [];
// Drone is stored within the ship object when active: ship.drone
...
// Power-Up Types - ADDED NEW TYPES
const POWERUP_TYPES = {
    TEMP_SHIELD: 0,
    RAPID_FIRE: 1,
    EMP_BURST: 2,
    SCORE_MULT: 3, // ADDED
    DRONE: 4,      // ADDED
    INVINCIBILITY: 5 // ADDED
};
const NUM_POWERUP_TYPES = 6; // INCREASED
...
let turretWeight = 0;
let swarmerWeight = 0;
let powerUpSpawnRate = 0.0015; // Maybe slightly increase spawn rate with more types?
...
function setupShopButtons() {
    ...
    let startY = height / 2 - (isMobile ? 175 : 210);
...
function drawStyledButton(button) {
    ...
    switch (button.id) {
        ...
        case 'chargeShot': cost = ship.getUpgradeCost('chargeShot'); isMaxed = (cost === "MAX"); if (!isMaxed) canAfford = (money >= cost); currentLevelText = `Lvl ${ship.chargeShotLevel}/${ship.maxChargeLevel}`; label = `Charge Shot ${currentLevelText}`; isUpgradeButton = true; break;
...
// --- Main Game Logic ---
// MODIFIED: Update drone
function runGameLogic() {
    if (isPaused) { // Static drawing when paused
        if (ship) ship.draw();
        if (ship && ship.droneActive) ship.drone.draw(); // ADDED: Draw drone if paused
        for (let b of bullets) b.draw(); for (let cs of chargeShots) cs.draw(); for (let m of homingMissiles) m.draw(); for (let l of laserBeams) l.draw();
        for (let p of particles) p.draw(); for (let a of asteroids) a.draw(); for (let e of enemyShips) e.draw(); for (let eb of enemyBullets) eb.draw();
        for (let pt of potions) pt.draw(); for (let pu of powerUps) pu.draw(); displayHUD(); return;
    }

    if (!ship) return;

    ship.update(); // Includes power-up timers, drone state check
    if (ship.droneActive) ship.drone.update(); // ADDED: Update drone
    ship.draw(); // Includes invincibility visual
    if (ship.droneActive) ship.drone.draw(); // ADDED: Draw drone

    // Update & Draw Projectiles & Particles
    ...
    // Laser Damage Logic (unchanged from previous)
    ...
    handlePotions();
    handleCollisions(); // Includes score multiplier logic now
    handlePowerUps();   // Handles activation of new powerups

    // Combo Timer Update
    if (comboTimer > 0) { comboTimer--; if (comboTimer <= 0) { if (maxComboReached >= 3) { let bonusPoints = maxComboReached * 5; let bonusMoney = floor(maxComboReached / 3); /* Multiplier doesn't apply to combo bonus */ points += bonusPoints; money += bonusMoney; infoMessage = `Combo Bonus: +${bonusPoints} PTS, +$${bonusMoney}! (Max: x${maxComboReached})`; infoMessageTimeout = 120; } comboCounter = 0; maxComboReached = 0; showComboText = false; comboTextTimeout = 0; } }
    ...
    if (gameState === GAME_STATE.PLAYING) {
        ...
        // MODIFIED: Spawn logic includes new powerup types
...
function handlePowerUps() {
    ...
    for (let i = powerUps.length - 1; i >= 0; i--) {
        ...
        if (powerUps[i].hitsShip(ship)) {
            ...
            powerUps.splice(i, 1);
            createParticles(pickupPos.x, pickupPos.y, 25, pickupColor, 4, 1.8, 0.8); // Generic pickup particles

            switch (powerUpType) {
                case POWERUP_TYPES.TEMP_SHIELD:
                    ship.tempShieldActive = true; infoMessage = "TEMPORARY SHIELD!";
                    createParticles(ship.pos.x, ship.pos.y, 20, color(45, 90, 100));
                    break;
                case POWERUP_TYPES.RAPID_FIRE:
                    ship.rapidFireTimer = 300; infoMessage = "RAPID FIRE!";
                    createParticles(ship.pos.x, ship.pos.y, 20, color(120, 90, 100));
                    break;
                case POWERUP_TYPES.EMP_BURST:
                    infoMessage = "EMP BURST!"; createParticles(ship.pos.x, ship.pos.y, 60, color(210, 100, 100), 12, 3.5, 1.2);
                    for (let k = asteroids.length - 1; k >= 0; k--) { createParticles(asteroids[k].pos.x, asteroids[k].pos.y, 15, asteroids[k].color, 3, 1.5); } asteroids = [];
                    for (let k = enemyShips.length - 1; k >= 0; k--) { createParticles(enemyShips[k].pos.x, enemyShips[k].pos.y, 20, enemyShips[k].getExplosionColor()); } enemyShips = [];
                    break;
                // --- ADDED POWERUP ACTIVATION ---
                case POWERUP_TYPES.SCORE_MULT:
                    ship.scoreMultiplierTimer = 600; // Duration in frames (e.g., 10 seconds)
                    ship.scoreMultiplierValue = 2; // Multiplier value (e.g., 2x)
                    infoMessage = `SCORE x${ship.scoreMultiplierValue}!`;
                    createParticles(ship.pos.x, ship.pos.y, 30, color(60, 100, 100)); // Yellow particles
                    break;
                case POWERUP_TYPES.DRONE:
                     if (!ship.droneActive) { // Only activate if no drone is currently active
                         ship.droneActive = true;
                         ship.drone = new Drone(ship);
                         infoMessage = "DRONE ACTIVATED!";
                         createParticles(ship.pos.x, ship.pos.y, 30, color(180, 50, 100)); // Cyan/Blue particles
                     } else {
                         // Maybe extend drone duration or upgrade it slightly? For now, do nothing if already active.
                         infoMessage = "DRONE ALREADY ACTIVE!";
                     }
                    break;
                case POWERUP_TYPES.INVINCIBILITY:
                    ship.invincibilityTimer = 480; // Duration in frames (e.g., 8 seconds)
                    infoMessage = "INVINCIBLE!";
                    createParticles(ship.pos.x, ship.pos.y, 40, color(0, 0, 100)); // White/Bright particles
                    // Ensure other shield types are off if invincibility is picked up
                    ship.tempShieldActive = false;
                    ship.shieldCharges = 0;
                    break;
                 // --------------------------------
            }
            infoMessageTimeout = 120; // Show message
...
// MODIFIED: Incorporate score multiplier and invincibility check
function handleCollisions() {
    if (gameState !== GAME_STATE.PLAYING || !ship || isPaused) return;

    // --- Helper function for handling enemy destruction (Adds score multiplier) ---
    const destroyEnemy = (enemy, index, projectileType) => {
        ...
        if (ship.scoreMultiplierTimer > 0) {
             pointsToAdd *= ship.scoreMultiplierValue;
             // Add visual feedback for multiplied points? (e.g., different color text popup)
        }
        // -----------------------------
    ...
    for (let i = asteroids.length - 1; i >= 0; i--) {
        ...
        // Check regular bullets, charge shots, missiles... (collision checks unchanged)
        ...
        if (asteroidHit) {/* process hit & continue */} else {
            for (let j = chargeShots.length - 1; j >= 0; j--) { if (asteroids[i] && chargeShots[j] && asteroids[i].hits(chargeShots[j])) { createParticles(chargeShots[j].pos.x, chargeShots[j].pos.y, floor(random(8, 15)), chargeShots[j].color, 4, 1.5, 0.9); chargeShots[j].takeDamage(1); if (chargeShots[j].health <= 0) { chargeShots.splice(j, 1); } asteroidHit = true; break; } }
        }
        if (asteroidHit) {/* process hit & continue */} else {
            for (let j = homingMissiles.length - 1; j >= 0; j--) { if (asteroids[i] && homingMissiles[j] && asteroids[i].hits(homingMissiles[j])) { createParticles(homingMissiles[j].pos.x, homingMissiles[j].pos.y, 15, homingMissiles[j].color, 5, 1.8, 1.0); homingMissiles.splice(j, 1); asteroidHit = true; break; } }
        }

        if (asteroidHit) {
            let oldPoints = points;
            let asteroidSizeValue = asteroids[i].size;
            // --- Apply Score Multiplier to Asteroid Points ---
            let pointsToAdd = floor(map(asteroidSizeValue, minAsteroidSize, 80, 5, 15));
             if (ship.scoreMultiplierTimer > 0) {
                 pointsToAdd *= ship.scoreMultiplierValue;
             }
            points += pointsToAdd;
            ...
            // Check Level Up / Win Condition (unchanged)
             if (currentLevel < MAX_LEVEL && points >= LEVEL_THRESHOLDS[currentLevel]) { points += 100 * currentLevel; money += 25 * currentLevel; gameState = GAME_STATE.UPGRADE_SHOP; infoMessage = `Level ${currentLevel} Cleared!`; infoMessageTimeout = 180; setupShopButtons(); cursor(ARROW); bullets = []; chargeShots = []; homingMissiles = []; laserBeams = []; enemyShips = []; enemyBullets = []; powerUps = []; potions = []; comboCounter = 0; comboTimer = 0; maxComboReached = 0; showComboText = false; comboTextTimeout = 0; return; }
             else if (currentLevel === MAX_LEVEL && points >= LEVEL_THRESHOLDS[currentLevel]) { gameState = GAME_STATE.WIN_SCREEN; infoMessage = ""; infoMessageTimeout = 0; cursor(ARROW); bullets = []; chargeShots = []; homingMissiles = []; laserBeams = []; asteroids = []; particles = []; enemyShips = []; enemyBullets = []; powerUps = []; potions = []; comboCounter = 0; comboTimer = 0; maxComboReached = 0; showComboText = false; comboTextTimeout = 0; return; }
        }
    }

    // --- Projectile vs Enemy Collisions --- (Uses destroyEnemy helper now)
    for (let i = enemyShips.length - 1; i >= 0; i--) {
        ...
        for (let j = chargeShots.length - 1; j >= 0; j--) { if (chargeShots[j] && enemy.hits(chargeShots[j])) { createParticles(chargeShots[j].pos.x, chargeShots[j].pos.y, 10, chargeShots[j].color, 4); let damage = chargeShots[j].damage; chargeShots[j].takeDamage(1); if (chargeShots[j].health <= 0) { chargeShots.splice(j, 1); } if (enemy.takeDamage(damage)) { destroyEnemy(enemy, i, 'ChargeShot'); enemyDestroyed = true; } else { createParticles(enemy.pos.x, enemy.pos.y, 5, enemy.getHitColor(), 3); } break; } }
        if (enemyDestroyed) continue;
        for (let j = homingMissiles.length - 1; j >= 0; j--) { if (homingMissiles[j] && enemy.hits(homingMissiles[j])) { createParticles(homingMissiles[j].pos.x, homingMissiles[j].pos.y, 20, homingMissiles[j].color, 6, 2.0, 1.2); homingMissiles.splice(j, 1); if (enemy.takeDamage(homingMissiles[0]?.damage || 3)) { destroyEnemy(enemy, i, 'HomingMissile'); enemyDestroyed = true; } else { createParticles(enemy.pos.x, enemy.pos.y, 8, enemy.getHitColor(), 4); } break; } }
        if (enemyDestroyed) continue;
    }


    // --- Player Ship vs Hazards Collisions ---
    // NOTE: Using ship.invulnerableTimer for post-hit brief invulnerability
    // AND ship.invincibilityTimer for the longer power-up invincibility.
    if (ship.invulnerableTimer <= 0) { // Check brief invulnerability first

        // --- ADDED: Invincibility Power-up Check ---
        if (ship.invincibilityTimer > 0) {
            // Check for collisions to create a visual effect, but don't take damage
            let hitHazard = null;
            let hitSourceArray = null;
            let hitIndex = -1;

            for (let i = asteroids.length - 1; i >= 0; i--) { if (asteroids[i] && asteroids[i].hitsShip(ship)) { hitHazard = asteroids[i]; hitSourceArray = asteroids; hitIndex = i; break; } }
            if (!hitHazard && gameState === GAME_STATE.PLAYING) { for (let i = enemyShips.length - 1; i >= 0; i--) { if (enemyShips[i] && enemyShips[i].hitsShip(ship)) { hitHazard = enemyShips[i]; hitSourceArray = enemyShips; hitIndex = i; break; } } }
            if (!hitHazard && gameState === GAME_STATE.PLAYING) { for (let i = enemyBullets.length - 1; i >= 0; i--) { if (enemyBullets[i] && enemyBullets[i].hitsShip(ship)) { hitHazard = enemyBullets[i]; hitSourceArray = enemyBullets; hitIndex = i; break; } } }

            if (hitHazard) {
                 // Create deflection particles
                 createParticles(ship.pos.x, ship.pos.y, 15, color(0,0,100), 4, 2.0);
                 // Destroy the hazard (optional, makes invincibility very strong)
                  if(hitSourceArray && hitIndex > -1 && hitSourceArray.includes(hitHazard)) {
                      let explosionColor = (hitHazard.getExplosionColor) ? hitHazard.getExplosionColor() : (hitHazard.color || color(0,0,50));
                      let particleCount = hitHazard.size ? floor(hitHazard.size * 0.5) : 10;
                      createParticles(hitHazard.pos.x, hitHazard.pos.y, particleCount, explosionColor, hitHazard.size * 0.15);
                       if (!(hitHazard instanceof KamikazeEnemy)) {
                          hitSourceArray.splice(hitIndex, 1);
                      } else {
                           // Kamikaze explodes anyway
                           createParticles(hitHazard.pos.x, hitHazard.pos.y, floor(hitHazard.size * 1.5), hitHazard.getExplosionColor(), hitHazard.size * 0.3, 1.5, 1.2);
                           hitSourceArray.splice(hitIndex, 1);
                      }
                  }
                 return; // Exit collision checks for player, invincible!
            }
             // If no collision while invincible, just continue the loop
        }
        // --- End Invincibility Power-up Check ---


        // --- Regular Damage Check (if not invincible) ---
        const takeDamage = (sourceObject, sourceArray, index) => {
            // --- Double check invincibility timers ---
            ...
            if (ship.tempShieldActive) { // Temp Shield Power-up
            ...
            } else if (ship.shieldCharges > 0) { // Regular Shield Charges
                ...
                if (sourceObject && sourceArray && index !== undefined) {
                    ...
                    if (!(sourceObject instanceof KamikazeEnemy)) { if (sourceArray && sourceArray.includes(sourceObject)) { sourceArray.splice(index, 1); } }
                        ...
                        if (sourceArray && sourceArray.includes(sourceObject)) { sourceArray.splice(index, 1); }
                    }
                }
            }
            return gameOver;
...
// --- HUD & Info Messages ---
// MODIFIED: Added Score Multiplier Indicator
function displayHUD() {
    ...
    // --- ADDED: Score Multiplier Indicator ---
    if (ship && ship.scoreMultiplierTimer > 0) {
         let multColor = color(60, 100, 100); // Yellow
         let alpha = map(sin(frameCount*0.2), -1, 1, 70, 100); // Pulsing alpha
         fill(hue(multColor), saturation(multColor), brightness(multColor), alpha);
         let indicatorText = `x${ship.scoreMultiplierValue}`;
         text(indicatorText, currentX, hudH / 2);
         // Draw a small pulsing circle around it maybe?
         noFill();
         stroke(hue(multColor), saturation(multColor), brightness(multColor), alpha * 0.5);
         strokeWeight(1.5);
         ellipse(currentX + textWidth(indicatorText)/2, hudH/2, iconSize*1.2, iconSize*1.2);
         currentX += textWidth(indicatorText) + spacing * 1.5;
    }
    // ---------------------------------------


    // Bottom Right Text (Weapon Levels)
    ...
    if (ship.chargeShotLevel > 0) upgradesText += ` CHRG:${ship.chargeShotLevel}`; if (ship.rearGunLevel > 0) upgradesText += ` REAR:${ship.rearGunLevel}`; if (ship.laserBeamLevel > 0) upgradesText += ` LASR:${ship.laserBeamLevel}`;
    text(upgradesText, width - sideMargin, height - bottomMargin);

    // Mobile settings button
    if (isMobile && gameState === GAME_STATE.PLAYING && !isPaused) { let btn = mobileSettingsButton; fill(uiPanelColor); stroke(uiBorderColor); strokeWeight(1.5); rect(btn.x, btn.y, btn.size, btn.size, 5); push(); translate(btn.x + btn.size / 2, btn.y + btn.size / 2); noFill(); stroke(uiTextColor); strokeWeight(2); ellipse(0, 0, btn.size * 0.5, btn.size * 0.5); for (let i = 0; i < 6; i++) { rotate(PI / 3); rect(-btn.size * 0.1, -btn.size * 0.4, btn.size * 0.2, btn.size * 0.2); } pop(); }
}
...
// --- Game State Control ---
// MODIFIED: Reset new power-up states
function resetGame() {
    ship = new Ship();
    bullets = []; chargeShots = []; homingMissiles = []; laserBeams = [];
    particles = []; asteroids = []; potions = []; enemyShips = []; enemyBullets = [];
    powerUps = []; nebulas = []; shootingStars = [];
    points = 0; money = 0; lives = 3; currentLevel = 1;
    setDifficultyForLevel(currentLevel);
    lastPlanetAppearanceTime = -Infinity; planetVisible = false; frameCount = 0;
    infoMessage = ""; infoMessageTimeout = 0; screenShakeDuration = 0; screenShakeIntensity = 0;
...
function startNextLevel() {
    ...
    ship.resetPositionForNewLevel(); // Resets basic states + new powerup states
    if (ship.homingMissilesLevel > 0) { ship.currentMissiles = ship.maxMissiles; } // Replenish missiles
    asteroids = []; bullets = []; chargeShots = []; homingMissiles = []; laserBeams = [];
    enemyShips = []; enemyBullets = []; powerUps = []; potions = [];
...
// --- Input Handling --- (Unchanged from previous version)
function mousePressed() {
    ...
    else if (gameState === GAME_STATE.START_MENU) { for (let i = 0; i < startMenuButtons.length; i++) { let button = startMenuButtons[i]; if (mouseX > button.x && mouseX < button.x + button.w && mouseY > button.y && mouseY < button.y + button.h) { selectedMenuItem = i; selectMenuItem(i); return; } } }
    else if (gameState === GAME_STATE.SETTINGS_MENU) { for (let i = 0; i < settingsMenuButtons.length; i++) { let button = settingsMenuButtons[i]; if (mouseX > button.x && mouseX < button.x + button.w && mouseY > button.y && mouseY < button.y + button.h) { selectedSettingsItem = i; selectSettingsItemAction(i); return; } } }
    else if (gameState === GAME_STATE.PLAYING && !isPaused) { if (!isMobile || !(mouseX > btn.x && mouseX < btn.x + btn.size + btn.padding*2 && mouseY > btn.y && mouseY < btn.y + btn.size + btn.padding*2)) { if (ship.chargeShotLevel > 0) { ship.startCharging(); } else { ship.shoot(); } } }
...
function keyPressed() {
    ...
    else if (gameState === GAME_STATE.PLAYING && !isPaused && ship) { if (keyCode === 32) { if (!spacebarHeld) { if (ship.chargeShotLevel > 0) { ship.startCharging(); } else { ship.shoot(); } spacebarHeld = true; } return false; } if (keyCode === 77) { ship.fireMissile(); return false; } if (keyCode === 76) { ship.fireLaser(); return false; } }
...
function keyReleased() { if (keyCode === 32) { spacebarHeld = false; if (gameState === GAME_STATE.PLAYING && !isPaused && ship && ship.isCharging) { ship.releaseChargeShot(); } } }
function touchStarted() { if (touches.length === 0) return false; let touchX = touches[0].x; let touchY = touches[0].y; let btn = mobileSettingsButton; if (gameState === GAME_STATE.PLAYING && !isPaused && isMobile && touchX > btn.x && touchX < btn.x + btn.size + btn.padding*2 && touchY > btn.y && touchY < btn.y + btn.size + btn.padding*2) { isPaused = true; previousGameState = gameState; gameState = GAME_STATE.SETTINGS_MENU; selectedSettingsItem = 0; cursor(ARROW); return false; } else if (gameState === GAME_STATE.START_MENU) { for (let i = 0; i < startMenuButtons.length; i++) { let button = startMenuButtons[i]; if (touchX > button.x && touchX < button.x + button.w && touchY > button.y && touchY < button.y + button.h) { selectedMenuItem = i; selectMenuItem(i); return false; } } return false; } else if (gameState === GAME_STATE.SETTINGS_MENU) { for (let i = 0; i < settingsMenuButtons.length; i++) { let button = settingsMenuButtons[i]; if (touchX > button.x && touchX < button.x + button.w && touchY > button.y && touchY < button.y + button.h) { selectedSettingsItem = i; selectSettingsItemAction(i); return false; } } return false; } else if (gameState === GAME_STATE.GAME_OVER || gameState === GAME_STATE.WIN_SCREEN) { previousGameState = gameState; gameState = GAME_STATE.START_MENU; selectedMenuItem = 0; return false; } else if (gameState === GAME_STATE.UPGRADE_SHOP) { for (let button of shopButtons) { if (touchX > button.x && touchX < button.x + button.w && touchY > button.y && touchY < button.y + button.h) { handleShopButtonPress(button.id); return false; } } return false; } else if (gameState === GAME_STATE.PLAYING && !isPaused) { if (!isMobile || !(touchX > btn.x && touchX < btn.x + btn.size + btn.padding*2 && touchY > btn.y && touchY < btn.y + btn.size + btn.padding*2)) { if (ship.chargeShotLevel > 0) { ship.startCharging(); } else { ship.shoot(); } } return false; } return false; }
...
// ==================
// Ship Class (MODIFIED for new power-ups)
// ==================
class Ship {
    constructor() {
        this.pos = createVector(width / 2, height - 50); this.vel = createVector(0, 0); this.thrust = 0.38; this.touchThrustMultiplier = 1.15; this.friction = 0.975; this.maxSpeed = 9.5; this.size = 30;
        ...
        this.shapeState = 0; this.shootCooldown = 0; this.baseShootDelay = 15; this.shootDelayPerLevel = 2; this.shieldCharges = 0; this.shieldVisualRadius = this.size * 1.3;
        this.invulnerableTimer = 0; this.invulnerabilityDuration = 120; // Brief post-hit invulnerability
        this.hoverOffset = 0; this.rapidFireTimer = 0; this.tempShieldActive = false;
        this.fireRateLevel = 0; this.spreadShotLevel = 0; this.maxUpgradeLevel = 5;
        this.chargeShotLevel = 0; this.maxChargeLevel = 3; this.chargeLevel = 0; this.maxChargeTime = [0, 60, 50, 40]; this.chargeDamage = [0, 3, 5, 8]; this.chargeSizeMult = [0, 1.5, 2.0, 2.5]; this.isCharging = false; this.chargeColor = color(90, 100, 100);
        ...
        // --- ADDED: New Power-up State Variables ---
        this.scoreMultiplierTimer = 0;
        this.scoreMultiplierValue = 1;
        this.droneActive = false;
        this.drone = null; // Will hold the Drone object
        this.invincibilityTimer = 0; // Longer invincibility from power-up
        // -----------------------------------------
    }

    // Getters & Basic Methods (unchanged)
    ...
    setInvulnerable() { this.invulnerableTimer = this.invulnerabilityDuration; } // Sets the BRIEF post-hit invulnerability
    ...
    // Upgrade Costs & Handling (unchanged)
    ...
    // MODIFIED: Reset new powerup states
    resetPositionForNewLevel() {
        this.pos.set(width / 2, height - 50); this.vel.set(0, 0);
        this.invulnerableTimer = 60; // Brief invulnerability at start
        this.rapidFireTimer = 0; this.tempShieldActive = false;
        this.isCharging = false; this.chargeLevel = 0;
        this.laserActive = false; this.laserDuration = 0; this.laserCooldown = 0;
    ...
    // --- Update Method ---
    // MODIFIED: Handle new power-up timers, drone state
    update() {
        // Basic Timers
        ...
        // --- New Power-up Timers ---
        if (this.scoreMultiplierTimer > 0) { this.scoreMultiplierTimer--; if(this.scoreMultiplierTimer <= 0) this.scoreMultiplierValue = 1;}
        if (this.invincibilityTimer > 0) { this.invincibilityTimer--; }
        // Drone has its own timer/logic within its class, but we check if it should be deactivated
        if (this.droneActive && this.drone && this.drone.isExpired()) {
             this.droneActive = false;
             this.drone = null;
             infoMessage = "Drone Deactivated";
             infoMessageTimeout = 90;
             // Add deactivation particles?
             createParticles(this.pos.x, this.pos.y, 15, color(180, 50, 80), 4, 1.5, 0.8);
        }
        // ---------------------------
        ...
        // Charging Logic
        if (this.isCharging) { this.chargeLevel = min(this.chargeLevel + 1, this.maxChargeTime[this.chargeShotLevel]); if (frameCount % 5 === 0) { let chargeRatio = this.chargeLevel / this.maxChargeTime[this.chargeShotLevel]; createParticles(this.pos.x + random(-5,5), this.pos.y - this.size * 0.6, 1, this.chargeColor, 2 + chargeRatio * 4, 0.5 * chargeRatio, 0.3); } }
    ...
    fireLaser() { if (this.laserBeamLevel > 0 && !this.laserActive && this.laserCooldown <= 0 && !this.isCharging) { this.laserActive = true; this.laserDuration = this.laserDurationTime[this.laserBeamLevel]; laserBeams.push(new LaserBeam(this, this.laserColor, 5 + this.laserBeamLevel * 1.5)); createParticles(this.pos.x, this.pos.y - this.size*0.7, 30, this.laserColor, 5, 1.0, 0.5); } }
    ...
    // --- Draw Method ---
    // MODIFIED: Add invincibility visual
    draw() {
        // Determine if showing invincibility effect (post-hit OR power-up)
         let showInvulnerableEffect = this.invulnerableTimer > 0 || this.invincibilityTimer > 0;
         let drawShip = !showInvulnerableEffect || (showInvulnerableEffect && frameCount % 10 < 5); // Flicker effect

        if (drawShip) {
            push();
            translate(this.pos.x, this.pos.y + this.hoverOffset);

            // --- ADDED: Invincibility Power-up Visual ---
            if (this.invincibilityTimer > 0) {
                 let invincibilityAlpha = map(sin(frameCount * 0.5), -1, 1, 40, 90); // Faster pulse
                 let invincibilityColor = color(0, 0, 100); // White/Bright
                 fill(invincibilityColor, invincibilityAlpha);
                 noStroke();
                 ellipse(0, 0, this.shieldVisualRadius * 2.5, this.shieldVisualRadius * 2.5); // Large aura
                 strokeWeight(3);
                 stroke(invincibilityColor, invincibilityAlpha + 20);
                 noFill();
                 ellipse(0, 0, this.shieldVisualRadius * 2.5, this.shieldVisualRadius * 2.5);
            }
            // --- End Invincibility Visual ---
             // Regular Shield Visuals (only draw if NOT fully invincible)
...
// ==================
// Projectile Classes (Bullet, ChargeShot, HomingMissile, LaserBeam - Unchanged from previous)
// ==================
...
class ChargeShot { constructor(x, y, size, damage, speed, color) { this.pos = createVector(x, y); this.size = size; this.damage = damage; this.speed = speed; this.color = color; this.vel = createVector(0, -this.speed); this.health = 3; this.piercing = true; this.rotation = 0; } update() { this.pos.add(this.vel); this.rotation += 0.1; } draw() { push(); translate(this.pos.x, this.pos.y); rotate(this.rotation); noStroke(); let glowSize = this.size * 1.8; fill(hue(this.color), saturation(this.color), brightness(this.color), 40); ellipse(0, 0, glowSize, glowSize); fill(this.color); ellipse(0, 0, this.size, this.size); fill(0, 0, 100); ellipse(0, 0, this.size * 0.4, this.size * 0.4); pop(); } takeDamage(amount) { this.health -= amount; } isOffscreen() { let margin = this.size; return (this.pos.y < -margin); } hits(target) { let d = dist(this.pos.x, this.pos.y, target.pos.x, target.pos.y); return d < this.size / 2 + target.size / 2; } }
...
// ==================
// PowerUp Class (MODIFIED for new types)
// ==================
class PowerUp {
    constructor(type) {
        this.type = type;
...
class Drone {
    constructor(playerShip) {
        this.ship = playerShip; // Reference to the player ship
        this.offset = createVector(this.ship.size * 1.8, 0); // Desired position relative to ship
        this.pos = p5.Vector.add(this.ship.pos, this.offset); // Initial position
        this.vel = createVector();
        this.size = 15;
        this.color = color(180, 60, 95); // Cyan/Blue color
        this.wingColor = color(180, 40, 70);
        this.shootCooldown = 0;
        this.shootDelay = 35; // How often the drone shoots (frames)
        this.lifespan = 900; // Duration in frames (e.g., 15 seconds)
        this.rotation = 0;
        this.targetAngle = 0;
        this.lerpFactor = 0.1; // How quickly it follows the ship
    }

    update() {
        this.lifespan--;

        // Movement: Orbit/Follow the ship
        let targetPos = p5.Vector.add(this.ship.pos, this.offset.copy().rotate(this.rotation));
        this.pos.lerp(targetPos, this.lerpFactor);
        this.rotation += 0.02; // Rotate the offset position

        // Shooting
        this.shootCooldown--;
        if (this.shootCooldown <= 0 && !isPaused && gameState === GAME_STATE.PLAYING) {
             this.shoot();
    ...
    shoot() {
        // Simple forward shot for now
        bullets.push(new Bullet(this.pos.x, this.pos.y, 0)); // Drone bullet uses standard Bullet class
        createParticles(this.pos.x, this.pos.y, 2, this.color, 2, 1.2, 0.4);
    }

    draw() {
        push();
        translate(this.pos.x, this.pos.y);
        // Rotate based on ship position or movement? Simple rotation for now
        ...
        // Wings
        fill(this.wingColor);
        triangle(-s * 0.4, -s * 0.2, -s * 1.1, -s * 0.5, -s * 0.8, s * 0.4);
        triangle(s * 0.4, -s * 0.2, s * 1.1, -s * 0.5, s * 0.8, s * 0.4);

        // "Eye"
        fill(0, 0, 100);
        ellipse(0, -s * 0.3, s * 0.3, s * 0.3);
