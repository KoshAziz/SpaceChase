...
// - Ship Upgrade System (Manual Purchase in Shop: Fire Rate, Spread Shot) - Uses Money // MODIFIED (Manual Shop, No Auto-Fire)
...
// - Spacebar/Tap to shoot always enabled (Hold-to-shoot removed). // MODIFIED (Auto-Fire Removed)
...
// - REMOVED Auto-Fire Upgrade functionality entirely. // MODIFIED
// --------------------------
...
// Define buttons for the upgrade shop (Auto-Fire removed)
function setupShopButtons() { shopButtons = []; let buttonWidth = 200; let buttonHeight = 50; let startY = height / 2 - 80; // Adjusted start Y
 let spacing = 70; shopButtons.push({ id: 'fireRate', x: width / 2 - buttonWidth / 2, y: startY, w: buttonWidth, h: buttonHeight }); shopButtons.push({ id: 'spreadShot', x: width / 2 - buttonWidth / 2, y: startY + spacing, w: buttonWidth, h: buttonHeight }); // Auto-Fire button removed
 shopButtons.push({ id: 'nextLevel', x: width / 2 - buttonWidth / 2, y: startY + spacing * 2 + 20, w: buttonWidth, h: buttonHeight }); // Adjusted Y for next level button
}
...
function displayPauseScreen() { fill(0, 0, 0, 50); rect(0, 0, width, height); fill(0, 0, 100); textSize(64); textAlign(CENTER, CENTER); text("PAUSED", width / 2, height / 2 - 20); textSize(22); text("Press ESC to Resume", width / 2, height / 2 + 40); }
function displayUpgradeShop() {
    fill(240, 50, 15, 90); rect(0, 0, width, height); fill(0, 0, 100); textSize(48); textAlign(CENTER, TOP); text(`Level ${currentLevel} Complete!`, width / 2, 50); textSize(32); text("Upgrade Shop", width / 2, 110); textSize(24); textAlign(CENTER, TOP); text(`Money: $${money}`, width / 2, 160); textSize(16); textAlign(CENTER, CENTER);
    for (let button of shopButtons) {
        ...
        // Removed Auto-Fire case
        else if (button.id === 'nextLevel') { label = `Start Level ${currentLevel + 1}`; isMaxed = false; canAfford = true; }

        let buttonColor = color(200, 70, 50); let textColor = color(0, 0, 100);
        if (button.id !== 'nextLevel') { if (isMaxed) { buttonColor = color(0, 0, 50); textColor = color(0, 0, 80); label += " (MAX)"; } else if (!canAfford) { buttonColor = color(0, 70, 40); textColor = color(0, 0, 90); label += ` ($${cost})`; } else { buttonColor = color(120, 70, 50); label += ` ($${cost})`; } }
        else { buttonColor = color(90, 70, 60); }

        fill(buttonColor); stroke(0, 0, 80); strokeWeight(2); rect(button.x, button.y, button.w, button.h, 5); fill(textColor); noStroke(); text(label, button.x + button.w / 2, button.y + button.h / 2);
    }
}
...
function runGameLogic() {
  ...
  if (gameState === GAME_STATE.PLAYING) {
      ...
      let maxAsteroidsAllowed = min(40, 15 + currentLevel * 3); let maxEnemiesAllowed = min(8, 2 + floor(currentLevel / 2)); let maxPotionsAllowed = 2; let maxPowerUpsAllowed = 1; let maxNebulasAllowed = 2;
...
function handlePowerUps() {
    ...
    for (let i = powerUps.length - 1; i >= 0; i--) {
        if (powerUps[i].hitsShip(ship)) {
            let powerUpType = powerUps[i].type; let pickupPos = powerUps[i].pos.copy(); powerUps.splice(i, 1);
            switch (powerUpType) {
                case POWERUP_TYPES.TEMP_SHIELD: ship.tempShieldActive = true; infoMessage = "TEMPORARY SHIELD!"; createParticles(pickupPos.x, pickupPos.y, 20, color(45, 90, 100)); break; // TODO: Play powerup sound (Shield)
                case POWERUP_TYPES.RAPID_FIRE: ship.rapidFireTimer = 300; infoMessage = "RAPID FIRE!"; createParticles(pickupPos.x, pickupPos.y, 20, color(120, 90, 100)); break; // TODO: Play powerup sound (Rapid Fire)
                case POWERUP_TYPES.EMP_BURST: infoMessage = "EMP BURST!"; createParticles(ship.pos.x, ship.pos.y, 50, color(210, 100, 100), 10, 3); for (let k = asteroids.length - 1; k >= 0; k--) { createParticles(asteroids[k].pos.x, asteroids[k].pos.y, 10, asteroids[k].color); } asteroids = []; for (let k = enemyShips.length - 1; k >= 0; k--) { createParticles(enemyShips[k].pos.x, enemyShips[k].pos.y, 15, color(300, 80, 90)); } enemyShips = []; break; // TODO: Play powerup sound (EMP)
            }
...
function handleCollisions() {
    ...
    for (let i = asteroids.length - 1; i >= 0; i--) {
        ...
        for (let j = bullets.length - 1; j >= 0; j--) {
            if (asteroids[i] && bullets[j] && asteroids[i].hits(bullets[j])) { // TODO: Play asteroid hit sound
                ...
                let currentAsteroid = asteroids[i]; let asteroidPos = currentAsteroid.pos.copy(); let asteroidColor = currentAsteroid.color; asteroids.splice(i, 1); bullets.splice(j, 1); createParticles(asteroidPos.x, asteroidPos.y, floor(asteroidSizeValue / 3), asteroidColor); // TODO: Play asteroid explosion sound
    ...
    for (let i = enemyShips.length - 1; i >= 0; i--) {
        ...
        for (let j = bullets.length - 1; j >= 0; j--) {
            if (enemyShips[i] && bullets[j] && enemyShips[i].hits(bullets[j])) { // TODO: Play enemy hit/explosion sound
                points += 20; money += 5; createParticles(enemyShips[i].pos.x, enemyShips[i].pos.y, 15, color(300, 80, 90), 3, 1.2); enemyShips.splice(i, 1); bullets.splice(j, 1); break;
            }
        }
    }
    ...
    if (ship.invulnerableTimer <= 0) {
        const takeDamage = (sourceObject, sourceArray, index) => { // TODO: Play damage/shield hit sound
            let gameOver = false;
            if (ship.tempShieldActive) { ship.tempShieldActive = false; createParticles(ship.pos.x, ship.pos.y, 30, color(45, 100, 100)); infoMessage = "TEMPORARY SHIELD LOST!"; infoMessageTimeout = 90; if (sourceObject && sourceArray && index !== undefined) { if (sourceObject instanceof EnemyBullet) { sourceArray.splice(index, 1); } } }
            else if (ship.shieldCharges > 0) { ship.loseShield(); createParticles(ship.pos.x, ship.pos.y, 25, color(180, 80, 100)); if (sourceObject && sourceArray && index !== undefined) { if (sourceObject instanceof EnemyBullet) { sourceArray.splice(index, 1); } } }
            else { // TODO: Play hull damage/life lost sound
                 lives--; createParticles(ship.pos.x, ship.pos.y, 30, color(0, 80, 100)); screenShakeIntensity = 5; screenShakeDuration = 15;
                 ...
                 if (sourceObject && sourceArray && index !== undefined) { let explosionColor = sourceObject.color || color(0,0,50); let particleCount = sourceObject.size ? floor(sourceObject.size / 2) : 15; if (sourceObject instanceof EnemyShip) { explosionColor = color(300, 80, 90); particleCount = 15; } else if (sourceObject instanceof EnemyBullet) { explosionColor = color(300, 80, 90); particleCount = 5; } createParticles(sourceObject.pos.x, sourceObject.pos.y, particleCount, explosionColor); sourceArray.splice(index, 1); }
            }
            return gameOver;
        };
...
function displayHUD() {
    let hudTextSize = 18; textSize(hudTextSize); fill(0, 0, 100, 80); noStroke();
    textAlign(LEFT, TOP); text("Points: " + points, 15, 15); text(`Money: $${money}`, 15, 40); text(`Lives: ${lives} / ${MAX_LIVES}`, 15, 65); text(`Shields: ${ship.shieldCharges} / ${MAX_SHIELD_CHARGES}`, 15, 90); text(`Level: ${currentLevel}`, 15, 115);
    textAlign(RIGHT, TOP); fill(0, 0, 100, 80); text(`Rate Lvl: ${ship.fireRateLevel}/${ship.maxLevel}`, width - 15, 15); text(`Spread Lvl: ${ship.spreadShotLevel}/${ship.maxLevel}`, width - 15, 40); // Removed Auto-Fire display
}
function displayInfoMessage() { fill(0, 0, 100); textSize(16); textAlign(CENTER, BOTTOM); text(infoMessage, width / 2, height - 20); }
function displayGameOver() { fill(0, 0, 0, 50); rect(0, 0, width, height); fill(0, 90, 100); textSize(60); textAlign(CENTER, CENTER); text("GAME OVER", width / 2, height / 3); fill(0, 0, 100); textSize(30); text("Final Points: " + points, width / 2, height / 3 + 60); textAlign(CENTER, CENTER); textSize(22); let pulse = map(sin(frameCount * 0.1), -1, 1, 60, 100); fill(0, 0, pulse); let restartInstruction = isMobile ? "Tap Screen to Restart" : "Click or Press Enter to Restart"; text(restartInstruction, width / 2, height * 0.7); cursor(ARROW); }
function resetGame() {
    ship = new Ship(); bullets = []; particles = []; asteroids = []; potions = []; enemyShips = []; enemyBullets = []; powerUps = []; nebulas = [];
...
function startNextLevel() {
    ...
    frameCount = 0; infoMessage = `Starting Level ${currentLevel}`; infoMessageTimeout = 90; levelTransitionFlash = 10;
...
function handleShopButtonPress(buttonId) {
    ...
    else { // Removed autoFire case
        let success = ship.attemptUpgrade(buttonId);
        if (success) { // TODO: Play upgrade success sound
             let button = shopButtons.find(b => b.id === buttonId); if(button) { createParticles(button.x + button.w / 2, button.y + button.h / 2, 15, color(120, 80, 100), 5, 1.5); }
...
class Ship {
    constructor() {
        this.pos = createVector(width / 2, height - 50); this.vel = createVector(0, 0); this.thrust = 0.38; this.touchThrustMultiplier = 1.1; this.friction = 0.98; this.maxSpeed = 9; this.size = 30;
        this.cockpitColor = color(180, 100, 100); this.engineColor1 = color(30, 100, 100); this.engineColor2 = color(0, 100, 100); this.finColor = color(220, 60, 70); this.detailColor = color(0, 0, 60);
        this.shapeState = 0; this.shootCooldown = 0; this.baseShootDelay = 15; this.shootDelayPerLevel = 2; this.shieldCharges = 0; this.shieldVisualRadius = this.size * 1.1; this.invulnerableTimer = 0; this.invulnerabilityDuration = 120;
        // Upgrades (Auto-Fire removed)
        this.maxLevel = 5; this.fireRateLevel = 0; this.spreadShotLevel = 0; // Removed autoFireLevel
        this.baseUpgradeCost = 30; this.costMultiplier = 2.0; // Removed autoFireCost
    ...
    attemptUpgrade(upgradeType) {
        ...
        if (currentLevel < maxLevelForType && money >= cost) {
            ...
            // Removed autoFire level increment
            return true;
        } else { return false; }
    }
    ...
    update() {
        ...
        let isTouching = touches.length > 0; let targetVel = createVector(0, 0);
        if (isTouching) { let touchPos = createVector(touches[0].x, touches[0].y); let direction = p5.Vector.sub(touchPos, this.pos); if (direction.magSq() > 1) { direction.normalize(); targetVel = direction.mult(this.maxSpeed * this.touchThrustMultiplier); } this.vel.lerp(targetVel, 0.15); }
        else { let movingUp = keyIsDown(UP_ARROW) || keyIsDown(87); let movingDown = keyIsDown(DOWN_ARROW) || keyIsDown(83); let movingLeft = keyIsDown(LEFT_ARROW) || keyIsDown(65); let movingRight = keyIsDown(RIGHT_ARROW) || keyIsDown(68); let acceleration = createVector(0, 0); if (movingUp) { acceleration.y -= this.thrust; } if (movingDown) { acceleration.y += this.thrust; } if (movingLeft) { acceleration.x -= this.thrust; } if (movingRight) { acceleration.x += this.thrust; } this.vel.add(acceleration); this.vel.mult(this.friction); }
        ...
        // Removed Auto-Fire shooting check: if (this.autoFireLevel > 0 && keyIsDown(32)) { ... }
    }

    shoot() {
        if (this.shootCooldown <= 0) {
            let bulletX = this.pos.x; let bulletY = this.pos.y - this.size * 0.6;
...
// Nebula Class
// ==================
class Nebula { constructor() { this.numEllipses = floor(random(8, 15)); this.ellipses = []; this.rotation = random(TWO_PI); this.rotationSpeed = random(-0.0005, 0.0005); this.baseAlpha = random(4, 10); let overallWidth = random(width * 0.5, width * 1.2); let overallHeight = random(height * 0.3, height * 0.6); if (random(1) < 0.5) { this.pos = createVector(-overallWidth / 2, random(height)); this.vel = createVector(random(0.05, 0.15), random(-0.02, 0.02)); } else { this.pos = createVector(width + overallWidth / 2, random(height)); this.vel = createVector(random(-0.15, -0.05), random(-0.02, 0.02)); } let h1 = random(240, 330); let h2 = (h1 + random(-40, 40)) % 360; this.color1 = color(h1, random(40, 70), random(20, 50)); this.color2 = color(h2, random(40, 70), random(20, 50)); for (let i = 0; i < this.numEllipses; i++) { this.ellipses.push({ pos: createVector(random(-overallWidth * 0.4, overallWidth * 0.4), random(-overallHeight * 0.4, overallHeight * 0.4)), w: random(overallWidth * 0.2, overallWidth * 0.6), h: random(overallHeight * 0.2, overallHeight * 0.6), alpha: this.baseAlpha * random(0.7, 1.3) }); } } update() { this.pos.add(this.vel); this.rotation += this.rotationSpeed; } draw() { push(); translate(this.pos.x, this.pos.y); rotate(this.rotation); noStroke(); for (let el of this.ellipses) { let inter = map(el.pos.x, -width * 0.4, width * 0.4, 0, 1); let c = lerpColor(this.color1, this.color2, inter); fill(hue(c), saturation(c), brightness(c), el.alpha); ellipse(el.pos.x, el.pos.y, el.w, el.h); } pop(); } isOffscreen() { let maxDimension = max(this.ellipses.reduce((maxR, el) => max(maxR, el.pos.mag() + max(el.w, el.h) / 2), 0), width * 0.6); let margin = maxDimension; return (this.pos.x < -margin || this.pos.x > width + margin || this.pos.y < -margin || this.pos.y > height + margin); } }
