...
// - Dynamic Parallax Star Background (with occasional planet, galaxy, black hole) // ENHANCED (Twinkle, Shooting Stars)
// - Enhanced Engine Thrust Effect (More reactive) // ENHANCED (Particles)
// - Asteroid Splitting
// - Player Lives (Max 3)
...
let particles = [];
let stars = [];
let shootingStars = []; // <-- ADDED for background effect
let potions = [];
let enemyShips = [];
let enemyBullets = [];
let powerUps = [];
...
let shootingStarSpawnRate = 0.001; // <-- ADDED Rate for shooting stars
...
function createParticles(x, y, count, particleColor, particleSize = null, speedMult = 1, lifespanMult = 1) {
    ...
    for (let i = 0; i < count; i++) {
        let pColor = color(
            (baseHue + random(-15, 15)) % 360, // More hue variation
...
function draw() {
  ...
  // Spawn shooting stars occasionally (only during gameplay)
  if (gameState === GAME_STATE.PLAYING && !isPaused && random(1) < shootingStarSpawnRate) {
      shootingStars.push(new ShootingStar());
  }

  drawBackgroundAndStars(); // Includes Nebulas & Shooting Stars now
...
function displayUpgradeShop() {
    ...
    for (let button of shopButtons) {
        ...
        if (button.id !== 'nextLevel') {
            ...
            else { buttonColor = hover ? color(120, 75, 65) : color(120, 70, 55); label += ` ($${cost})`; borderColor = color(120, 80, 80); } // Hover effect for affordable
        } else { buttonColor = hover ? color(90, 75, 70) : color(90, 70, 60); borderColor = color(90, 80, 85); } // Hover effect for next level
...
function runGameLogic() {
  ...
  if (gameState === GAME_STATE.PLAYING) {
      ...
      let maxAsteroidsAllowed = min(40, 15 + currentLevel * 3); let maxEnemiesAllowed = min(8, 2 + floor(currentLevel / 2)); let maxPotionsAllowed = 2; let maxPowerUpsAllowed = 1; let maxNebulasAllowed = 3; // Allow slightly more nebulas
...
function handlePowerUps() {
    ...
    for (let i = powerUps.length - 1; i >= 0; i--) {
        if (powerUps[i].hitsShip(ship)) {
            let powerUpType = powerUps[i].type; let pickupPos = powerUps[i].pos.copy();
            let pickupColor = powerUps[i].color; // Get color before splicing
            powerUps.splice(i, 1);
            // Add pickup particles
            createParticles(pickupPos.x, pickupPos.y, 25, pickupColor, 4, 1.8, 0.8); // Larger, faster particles for pickup
...
function handleCollisions() {
    ...
    for (let i = asteroids.length - 1; i >= 0; i--) {
        ...
        for (let j = bullets.length - 1; j >= 0; j--) {
            if (asteroids[i] && bullets[j] && asteroids[i].hits(bullets[j])) { // TODO: Play asteroid hit sound
                let impactParticleCount = floor(random(3, 6)); createParticles(bullets[j].pos.x, bullets[j].pos.y, impactParticleCount, color(60, 40, 100), 2, 0.8, 0.7); // Whiter sparks
                ...
                let currentAsteroid = asteroids[i]; let asteroidPos = currentAsteroid.pos.copy(); let asteroidColor = currentAsteroid.color; asteroids.splice(i, 1); bullets.splice(j, 1); createParticles(asteroidPos.x, asteroidPos.y, floor(asteroidSizeValue / 2.5), asteroidColor, null, 1.2, 1.1); // More explosion particles // TODO: Play asteroid explosion sound
    ...
    if (ship.invulnerableTimer <= 0) {
        const takeDamage = (sourceObject, sourceArray, index) => { // TODO: Play damage/shield hit sound
            let gameOver = false;
            if (ship.tempShieldActive) { ship.tempShieldActive = false; createParticles(ship.pos.x, ship.pos.y, 40, color(45, 100, 100), 5, 2.0); infoMessage = "TEMPORARY SHIELD LOST!"; infoMessageTimeout = 90; if (sourceObject && sourceArray && index !== undefined) { createParticles(sourceObject.pos.x, sourceObject.pos.y, 5, color(45,90,100)); if (sourceObject instanceof EnemyBullet) { sourceArray.splice(index, 1); } } } // Small particle burst at hit location for shield
            else if (ship.shieldCharges > 0) { ship.loseShield(); createParticles(ship.pos.x, ship.pos.y, 35, color(180, 80, 100), 4, 1.8); if (sourceObject && sourceArray && index !== undefined) { createParticles(sourceObject.pos.x, sourceObject.pos.y, 5, color(180,80,100)); if (sourceObject instanceof EnemyBullet) { sourceArray.splice(index, 1); } } } // Small particle burst at hit location for shield
            else { // TODO: Play hull damage/life lost sound
                 lives--; createParticles(ship.pos.x, ship.pos.y, 40, color(0, 90, 100), 5, 2.2); screenShakeIntensity = 7; screenShakeDuration = 20; // Increased shake
...
function handlePotions() {
    ...
    for (let i = potions.length - 1; i >= 0; i--) {
        ...
        if (potions[i].hitsShip(ship)) { // TODO: Play potion pickup sound
             createParticles(potions[i].pos.x, potions[i].pos.y, 20, color(0, 80, 100), 4, 1.5); // Pickup particles
...
function drawBackgroundAndStars() {
    ...
    for (let i = shootingStars.length - 1; i >= 0; i--) { // <-- Draw Shooting Stars
         if (gameState === GAME_STATE.PLAYING && !isPaused) shootingStars[i].update();
         shootingStars[i].draw();
         if (shootingStars[i].isDone()) { shootingStars.splice(i, 1); }
     }
    for (let star of stars) { if (gameState === GAME_STATE.PLAYING && !isPaused) star.update(); star.draw(); }
}
function drawBlackHole() { push(); let bhX = width * 0.8; let bhY = height * 0.2; let bhSize = width * 0.05; fill(0); noStroke(); ellipse(bhX, bhY, bhSize * 1.1, bhSize * 1.1); // Slightly larger black core
 let ringCount = 7; let maxRingSize = bhSize * 3.5; let minRingSize = bhSize * 1.2; noFill(); for (let i = 0; i < ringCount; i++) { let size = lerp(minRingSize, maxRingSize, i / (ringCount - 1)); let hue = lerp(0, 60, i / (ringCount - 1)) + random(-10, 10); let alpha = map(i, 0, ringCount - 1, 50, 3); let sw = map(i, 0, ringCount - 1, 1.5, 5); strokeWeight(sw); stroke(hue, 90, 90, alpha); ellipse(bhX + random(-sw, sw), bhY + random(-sw, sw), size * random(0.9, 1.1), size * random(0.9, 1.1)); } pop(); } // More rings, more variation
function drawGalaxy() { push(); let centerX = width / 2; let centerY = height / 2; let baseHue1 = 270; let baseHue2 = 200; let alphaVal = 2.5; let angle = frameCount * 0.0003; translate(centerX, centerY); rotate(angle); translate(-centerX, -centerY); noStroke(); fill(baseHue1, 50, 60, alphaVal); ellipse(centerX - width * 0.1, centerY - height * 0.1, width * 1.3, height * 0.35); fill(baseHue2, 60, 70, alphaVal); ellipse(centerX + width * 0.15, centerY + height * 0.05, width * 1.2, height * 0.45); fill((baseHue1 + baseHue2) / 2, 55, 65, alphaVal * 0.9); ellipse(centerX, centerY, width * 1.0, height * 0.55); pop(); } // Slightly larger/brighter
function drawPlanet() { push(); translate(planetPos.x, planetPos.y); noStroke(); fill(planetBaseColor); ellipse(0, 0, planetSize, planetSize); fill(planetDetailColor1); arc(0, 0, planetSize, planetSize, PI * 0.1, PI * 0.6, OPEN); arc(0, 0, planetSize * 0.8, planetSize * 0.8, PI * 0.7, PI * 1.2, OPEN); fill(planetDetailColor2); arc(0, 0, planetSize * 0.9, planetSize * 0.9, PI * 1.3, PI * 1.9, OPEN); noFill(); strokeWeight(planetSize * 0.06); stroke(hue(planetBaseColor), 20, 100, 20); ellipse(0, 0, planetSize * 1.06, planetSize * 1.06); pop(); } // Thicker ring
function displayHUD() {
    ...
    const drawHudText = (txt, x, y, alignment) => {
        ...
        if (alignment === LEFT) { rect(x - hudPadding / 2, y - hudPadding / 2, txtWidth + hudPadding, hudTextSize + hudPadding, 3); }
        else if (alignment === RIGHT) { rect(x + hudPadding / 2 - (txtWidth + hudPadding), y - hudPadding / 2, txtWidth + hudPadding, hudTextSize + hudPadding, 3); }
...
function displayInfoMessage() { fill(0, 0, 100); textSize(18); textAlign(CENTER, BOTTOM); fill(0,0,0,40); rect(width/2 - textWidth(infoMessage)/2 - 10, height - 45, textWidth(infoMessage) + 20, 30, 5); fill(0, 0, 100); text(infoMessage, width / 2, height - 25); } // Added background to info message
...
function resetGame() {
    ship = new Ship(); bullets = []; particles = []; asteroids = []; potions = []; enemyShips = []; enemyBullets = []; powerUps = []; nebulas = []; shootingStars = []; // Reset shooting stars too
...
function startNextLevel() {
    ...
    frameCount = 0; infoMessage = `Starting Level ${currentLevel}`; infoMessageTimeout = 90; levelTransitionFlash = 15; // Longer flash
...
function handleShopButtonPress(buttonId) {
    ...
    if (buttonId === 'nextLevel') { startNextLevel(); } // TODO: Play UI confirm/next level sound
        ...
        if (success) { // TODO: Play upgrade success sound
             let button = shopButtons.find(b => b.id === buttonId); if(button) { createParticles(button.x + button.w / 2, button.y + button.h / 2, 20, color(120, 80, 100), 6, 2.0, 0.8); } // Bigger particle burst
...
class Ship {
    constructor() {
        this.pos = createVector(width / 2, height - 50); this.vel = createVector(0, 0); this.thrust = 0.42; // Slightly higher thrust
         this.touchThrustMultiplier = 1.15; this.friction = 0.975; // Slightly less friction
         this.maxSpeed = 9.5; this.size = 30;
        this.cockpitColor = color(180, 100, 100); this.baseEngineColor1 = color(30, 100, 100); this.baseEngineColor2 = color(0, 100, 100); // Store base colors
        this.finColor = color(220, 60, 70); this.detailColor = color(0, 0, 60);
        this.shapeState = 0; this.shootCooldown = 0; this.baseShootDelay = 15; this.shootDelayPerLevel = 2; this.shieldCharges = 0; this.shieldVisualRadius = this.size * 1.2; // Slightly larger shield visual
    ...
    update() {
        ...
        // Hover animation
        this.hoverOffset = sin(frameCount * 0.05) * 2; // Slow, subtle bobbing
        ...
        if (isTouching) {
             ...
             if (direction.magSq() > 1) { direction.normalize(); let targetVel = direction.mult(this.maxSpeed * this.touchThrustMultiplier); this.vel.lerp(targetVel, 0.15); applyThrustParticles = true; } // Apply thrust directly towards touch

        } else {
             let movingUp = keyIsDown(UP_ARROW) || keyIsDown(87); let movingDown = keyIsDown(DOWN_ARROW) || keyIsDown(83); let movingLeft = keyIsDown(LEFT_ARROW) || keyIsDown(65); let movingRight = keyIsDown(RIGHT_ARROW) || keyIsDown(68);
             if (movingUp) { acceleration.y -= this.thrust; applyThrustParticles = true;}
             if (movingDown) { acceleration.y += this.thrust; applyThrustParticles = true;} // Allow backward thrust particles too? Maybe not.
        ...
        // Spawn thrust particles if accelerating
        if (applyThrustParticles && frameCount % 3 === 0) { // Limit particle spawn rate
            let particleVel = this.vel.copy().mult(-0.1).add(p5.Vector.random2D().mult(0.5)); // Opposite direction + spread
            let thrustColor = lerpColor(this.baseEngineColor1, color(60, 100, 100), this.fireRateLevel / this.maxLevel); // Shift to yellow/white
            createParticles(this.pos.x, this.pos.y + this.size * 0.5, 1, thrustColor, 3, 1.5, 0.5);
        }
    ...
    shoot() {
        if (this.shootCooldown <= 0) {
            // Determine bullet origin points based on spread level
            let originY = this.pos.y - this.size * 0.6 + this.hoverOffset;
            let originPoints = [createVector(this.pos.x, originY)]; // Default center origin

            if (this.spreadShotLevel >= 1 && this.spreadShotLevel <= 2) { // 3 shots
                let offset = this.size * 0.15;
                originPoints = [ createVector(this.pos.x - offset, originY + 5), createVector(this.pos.x, originY), createVector(this.pos.x + offset, originY + 5) ];
            } else if (this.spreadShotLevel >= 3 && this.spreadShotLevel <= 4) { // 3 shots wider
                let offset = this.size * 0.2;
                 originPoints = [ createVector(this.pos.x - offset, originY + 5), createVector(this.pos.x, originY), createVector(this.pos.x + offset, originY + 5) ];
            } else if (this.spreadShotLevel >= this.maxLevel) { // 5 shots
                let offset1 = this.size * 0.25; let offset2 = this.size * 0.1;
                originPoints = [ createVector(this.pos.x - offset1, originY + 8), createVector(this.pos.x - offset2, originY + 3), createVector(this.pos.x, originY), createVector(this.pos.x + offset2, originY + 3), createVector(this.pos.x + offset1, originY + 8) ];
            }
            ...
            for (let i = 0; i < numShots; i++) {
                let angle = 0; if (numShots > 1) { angle = map(i, 0, numShots - 1, -spreadAngle, spreadAngle); }
                // Use corresponding origin point if available, else default to center
    ...
    draw() { if (this.invulnerableTimer <= 0 || (this.invulnerableTimer > 0 && frameCount % 10 < 5) ) { push(); translate(this.pos.x, this.pos.y + this.hoverOffset); // Apply hover offset here
         if (this.tempShieldActive) { let tempShieldAlpha = map(sin(frameCount * 0.3), -1, 1, 60, 100); let tempShieldHue = 45; fill(tempShieldHue, 90, 100, tempShieldAlpha); noStroke(); ellipse(0, 0, this.shieldVisualRadius * 2.3, this.shieldVisualRadius * 2.3); strokeWeight(2.5); stroke(tempShieldHue, 100, 100, tempShieldAlpha + 25); noFill(); ellipse(0, 0, this.shieldVisualRadius * 2.3, this.shieldVisualRadius * 2.3); } // Brighter temp shield
         else if (this.shieldCharges > 0) { let shieldAlpha = map(sin(frameCount * 0.2), -1, 1, 50, 90); let shieldHue = 180; fill(shieldHue, 80, 100, shieldAlpha); noStroke(); ellipse(0, 0, this.shieldVisualRadius * 2.1, this.shieldVisualRadius * 2.1); strokeWeight(2); stroke(shieldHue, 90, 100, shieldAlpha + 35); noFill(); ellipse(0, 0, this.shieldVisualRadius * 2.1, this.shieldVisualRadius * 2.1); } // Brighter shield

        let enginePulseFactor = 1.0 + this.vel.mag() * 0.4; let pulseSpeed = (this.rapidFireTimer > 0) ? 0.5 : 0.25; let enginePulse = map(sin(frameCount * pulseSpeed), -1, 1, 0.8, 1.3) * enginePulseFactor; let engineSize = this.size * 0.55 * enginePulse; let engineBrightness = map(sin(frameCount * 0.35), -1, 1, 85, 100); noStroke();
        // Lerp engine colors based on fire rate level
...
class Bullet {
    constructor(x, y, angle = 0) {
        this.pos = createVector(x, y); this.speed = 17; this.size = 5.5; // Slightly larger
        this.startHue = frameCount % 360; this.hue = this.startHue;
    ...
    update() {
        // Add current position to trail start
        this.trail.unshift(this.pos.copy());
        // Remove oldest position if trail is too long
        if (this.trail.length > this.trailLength) { this.trail.pop(); }

        this.pos.add(this.vel); this.hue = (this.hue + 5) % 360; // Faster hue cycle
    }
    ...
    draw() {
        ...
        for (let i = 0; i < this.trail.length; i++) {
            ...
            let alpha = map(i, 0, this.trail.length - 1, 50, 0); // Fade out alpha
            let trailSize = map(i, 0, this.trail.length - 1, this.size, this.size * 0.5); // Shrink size
            fill(this.hue, 90, 100, alpha);
            ellipse(trailPos.x, trailPos.y, trailSize, trailSize * 2.0); // Shorter trail segments
        }

        // Draw Main Bullet Head
        fill(this.hue, 95, 100); stroke(0, 0, 100); strokeWeight(1);
        ellipse(this.pos.x, this.pos.y, this.size, this.size * 2.5);
    }

    isOffscreen() { let margin = this.size * 5; return (this.pos.y < -margin || this.pos.y > height + margin || this.pos.x < -margin || this.pos.x > width + margin); } // Larger margin
}
...
class Particle {
    constructor(x, y, particleColor, size = null, speedMult = 1, lifespanMult = 1) {
        ...
        this.vel.mult(random(1.5, 6) * speedMult); // Increased max speed
        this.lifespan = 100 * lifespanMult * random(0.8, 1.5); // More lifespan variation
        this.maxLifespan = this.lifespan; // Store initial lifespan for alpha calculation
        this.baseHue = hue(particleColor); this.baseSat = saturation(particleColor); this.baseBri = brightness(particleColor);
        this.size = size !== null ? size * random(0.8, 1.2) : random(2, 7); // More size variation
        this.drag = random(0.95, 0.99); // Varying drag
    }

    update() {
        this.pos.add(this.vel);
        this.lifespan -= 2.5; // Slower fade
        this.vel.mult(this.drag);
    }

    draw() {
        noStroke();
        let currentAlpha = map(this.lifespan, 0, this.maxLifespan, 0, 100); // Calculate alpha based on remaining lifespan
        fill(this.baseHue, this.baseSat, this.baseBri, currentAlpha);
        ellipse(this.pos.x, this.pos.y, this.size * (this.lifespan / this.maxLifespan)); // Shrink over time
    }
...
// Star Class
// ==================
class Star { constructor() { this.x = random(width); this.y = random(height); this.layer = floor(random(4)); // Added one more layer
 this.size = map(this.layer, 0, 3, 0.4, 2.8); this.speed = map(this.layer, 0, 3, 0.05, 0.6); this.baseBrightness = random(50, 95); this.twinkleSpeed = random(0.03, 0.08); this.twinkleRange = random(0.6, 1.4); this.twinkleOffset = random(TWO_PI); } update() { this.y += this.speed; if (this.y > height + this.size) { this.y = -this.size; this.x = random(width); } } draw() { let twinkleFactor = map(sin(frameCount * this.twinkleSpeed + this.twinkleOffset), -1, 1, 1.0 - this.twinkleRange / 2, 1.0 + this.twinkleRange / 2); let currentBrightness = constrain(this.baseBrightness * twinkleFactor, 30, 100); fill(0, 0, currentBrightness, 90); noStroke(); ellipse(this.x, this.y, this.size, this.size); } } // More twinkle variation


// ==================
// ShootingStar Class (NEW)
// ==================
class ShootingStar {
    constructor() {
        this.startX = random(width);
        this.startY = random(-50, -10);
        ...
        this.lifespan = 100; // Fade out duration
    }
    ...
    draw() {
        ...
        let tailPos = p5.Vector.sub(this.pos, this.vel.copy().setMag(this.len)); // Calculate tail position
        strokeWeight(random(1.5, 3));
        stroke(0, 0, this.brightness, alpha); // Whiteish color
...
class EnemyBullet {
    constructor(x, y, angle, speed) {
        this.pos = createVector(x, y);
        this.vel = p5.Vector.fromAngle(angle); this.vel.mult(speed);
        this.size = 7; // Slightly larger
        this.color = color(0, 90, 100); // Bright Red
    }
    ...
    hitsShip(ship) { let d = dist(this.pos.x, this.pos.y, ship.pos.x, ship.pos.y); let targetRadius = ship.tempShieldActive ? ship.shieldVisualRadius*1.1 : (ship.shieldCharges > 0 ? ship.shieldVisualRadius : ship.size * 0.5); return d < this.size * 0.6 + targetRadius; } // Adjusted hit radius slightly
